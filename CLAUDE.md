...
The workflow runtime is the Depot Agent product surface (PRD §4.3, §4.4). It is a single agent driven by per-workflow scoped tools and prompts. `WorkflowAgent.run_turn(workflow, depot_id, auth_context, tool_registry, user_input)` in `runtime.py` opens one Anthropic Messages tool-use loop: the model sees only the tools named in `workflow.allowed_tools` plus the reserved `emit_decision` structured-output terminator. The runtime validates every tool call against the allow-list **before** dispatching it (`ToolNotAllowedError` if not), records each call in the per-turn `tool_calls` list, runs the `HardConstraintGuard` (`constraints.py`) twice — pre-dispatch on tool inputs and post-emit on the LLM's `proposed_actions` (PRD §10.3: departure SoC ≥ 99%, `max_grid_kw` never exceeded) — and writes exactly one `Decision` row at the end with `disposition='pending'` via `DecisionRepo` (`repo.py`). The agent itself **never** writes `auto_executed`; humans, or a later promotion pathway, advance disposition. Prompt caching matches the `src/api/agent/llm.py` pattern: the workflow's system block carries `cache_control={"type": "ephemeral"}` so repeat turns of the same workflow hit Anthropic's prefix cache; the per-turn user message (depot id, parameters, user input) is kept out of the cached block. Prometheus metrics: `favonius_workflow_turns_total{workflow,depot,status}`, `favonius_workflow_turn_duration_seconds`, `favonius_workflow_llm_tokens_total`. **No HTTP endpoint yet** — the runtime is exercised via `tests/unit/test_agent_workflows_runtime.py` with fake tools and a fake Anthropic client. The `Workflow` and `Decision` Pydantic schemas live in `schemas.py` and carry the Sprint 1 contract forward inside this package.
