...
### Traffic-fine triage agent (`src/core/traffic_fines/` + `src/api/agent_workflows/traffic_fine.py` + `src/api/traffic_fines.py`)

An operator uploads a traffic-fine document (PDF/image); a Depot Agent **workflow** (`traffic_fine_triage`) reads it **multimodally** and the platform alerts the Logistics Manager when the early-payment discount is closing within <48h. Gated by `TRAFFIC_FINE_AGENT_ENABLED` (default off). This is the codebase's first multimodal LLM call.

- **Upload** — `POST /admin/depots/{id}/traffic-fines` (customer_admin/favonius_admin, depot-scoped). Raw-body upload (no multipart dependency), mirroring the charger-log endpoint: validate → magic-byte media detection (`src/core/traffic_fines/media.py`; PDF + PNG/JPEG/GIF/WEBP, unsupported → 415) → store bytes in `traffic_fines` (`status='received'`) → 202 `{id, status, status_url}`. Per-path body cap `TRAFFIC_FINE_UPLOAD_MAX_BYTES` (default 10 MiB) via `MaxBodySizeMiddleware`. Writes a `traffic_fine.uploaded` admin-audit row; triage runs as a post-commit `asyncio.create_task`.
- **Workflow** — `triage_traffic_fine(...)` builds a document/image content block and runs Sprint-2's `WorkflowAgent.run_turn(...)` at the `inform` tier with the document threaded through the runtime's new **`attachments`** hook (`runtime.py::_format_user_message` returns a content-block list when attachments are present — backward-compatible; no attachments → unchanged string). The LLM calls the single `record_fine_extraction` tool (schema = `TrafficFineExtraction.model_json_schema()`, `extra="forbid"`) then `emit_decision`; the extraction is captured verbatim in the immutable `Decision.tool_calls` (migration 037 audit). `decisions.workflow_id` FKs `workflows(id)`, so migration 047 seeds the `traffic_fine_triage` workflow row (fixed UUID `7f1ce0a0-…-047`).
- **Evaluation (deterministic, no LLM)** — `src/core/traffic_fines/evaluator.py::evaluate_early_payment(extraction, *, now, depot_tz, window_hours)` parses the deadline (date-only → end-of-day in the depot tz via `zoneinfo`; naive → depot tz; aware → honoured), computes `discount = full − early` (or a stated discount), and classifies `within_window | not_yet | expired | no_deadline | not_a_fine`. The alert message is built verbatim: `Priority: Early payment discount for Fine #[ID] expires in 2 days. Automate payment now to save €[Discount Amount]?` (the time phrase and currency symbol are derived from the data). Time math is never LLM-judged.
- **Alert (email only)** — `within_window` → `src/core/traffic_fines/alerting.py::raise_fine_alert` calls `upsert_alert` (`alert_type='traffic_fine_early_payment'`, severity `warning`, `dedup_key=traffic_fine_early_payment:{depot}:{ref-or-id}`); the existing `AlertDispatcher` (websocket_handler) delivers it on its ≤30 s poll. The **Logistics Manager** is a `notification_recipients` row subscribed to that type (no new role). Result + `decision_id`/`alert_id` persist on `traffic_fines` in the same transaction. There is no in-app proposed action / payment rail — alert-only by design.
- **Deadline sweep** — `src/core/traffic_fines/sweeper.py::run_traffic_fine_deadline_sweep` (FastAPI lifespan, gated by the flag, every `TRAFFIC_FINE_SWEEP_INTERVAL_S`, default 900 s) re-evaluates `status IN ('parsed','no_alert')` fines with a future deadline **using the stored extraction (no LLM)** and raises the alert when one crosses into the window — so a fine uploaded early still alerts. Idempotent via the alert dedup key + status flip to `alerted`.
- **Schema (migration 047, TimescaleDB)** — `traffic_fines` (raw `BYTEA` + parsed fields + evaluation + `decision_id`/`alert_id` links; bare depot/org UUIDs, no FK to Supabase). Reads: `GET /admin/depots/{id}/traffic-fines` (list) and `…/{fine_id}` (detail), any depot member.
...
- `traffic_fines` — Uploaded traffic-fine documents + triage results (migration 047). Raw `BYTEA` + LLM-extracted fields + deterministic evaluation + `decision_id`/`alert_id` links; bare depot/org UUIDs. See "Traffic-fine triage agent".
...
| `POST` | `/admin/depots/{id}/traffic-fines` | Upload a traffic-fine document (raw PDF/image body) for triage. customer_admin or favonius_admin; 202 + `status_url`; writes `traffic_fine.uploaded` audit (requires `TRAFFIC_FINE_AGENT_ENABLED=true`). |
| `GET` | `/admin/depots/{id}/traffic-fines` (+ `/{fine_id}`) | List / fetch uploaded fines with extracted fields, evaluation, and alert/decision links (any depot member). |
