...
### Depot Agent Workflows (readiness substrate)

`src/api/agent_workflows/` is the home for the deterministic multi-step workflows defined in the Depot Agent PRD (`docs/PRD_Depot_Agent.md` §6). Distinct from the chat agent above: that one answers a single SQL question per turn; workflows orchestrate several typed tool calls under a fixed prompt. The package exposes a process-global `ToolRegistry` (`tools/registry.py`) — workflows resolve their `allowed_tools` allowlist (PRD §5.2 dataclass) by name through this registry. Tools are registered as a side effect of importing their module (`@register_tool("name")`).

`src/api/agent_workflows/tools/readiness.py` ships the five tools the daily readiness workflow needs (PRD §6.1): `get_scheduled_departures`, `get_vehicle_state`, `get_charger_state`, `get_charging_plan`, `get_driver_assignment`. Each takes an `AuthContext` and filters by `visible_depot_ids` — the same auth boundary as `src/api/agent/resolve.py`. All SQL is parameterised; no string interpolation. Tools never raise on empty data — they return `[]` or `None`-bearing dicts with explicit freshness flags so the workflow orchestrator can reason about gaps instead of aborting the turn. The telemetry staleness threshold is `src.security.data_freshness.MAX_TELEMETRY_AGE` (15 minutes), the same constant `StateAssembler` enforces — the readiness tool re-uses it rather than redefining. The data plumbing this depends on: Supabase migration 013 (`schedules.driver_id`) and migration 014 (`routes` view aliasing `schedules` to the PRD §5.1 `Route` shape).
