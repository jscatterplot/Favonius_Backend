...
│   │   ├── navirec/             # Navirec telematics live SoC feed → vehicle_telemetry (client/mapping/poller)
│   │   ├── chargers/            # Charger-side log parsers (abb/log_parser.py) + vendor dispatch
│   │   ├── weather/             # OpenMeteo weather adapter
│   │   ├── rest_client.py       # BaseRestClient — shared auth/pagination/retry base (Kempower + Navirec)
│   │   └── handoff/
...
#### Two-model split (cost optimisation, PLAN.md §S5b — build-only spike)

A second cost lever, **on the consumption fast path** (not the SQL loop). When enabled for an org, the cheap "explore" extraction call (`src/api/agent/llm.py::extract_plan` — forced `tool_choice`, no adaptive thinking) routes to `claude-haiku-4-5` and the user-facing "format" reply (`format_answer`) routes to `claude-sonnet-4-6` (the prose a human reads never uses the cheap model). **When disabled — the default — both phases keep the configured single-model default (`CONFIG.model` / `AGENT_LLM_MODEL`), so the off path is a pure no-op** (an operator's non-default `AGENT_LLM_MODEL` still drives both calls). Routing lives in `src/api/agent/llm_router.py::pick_model(phase, *, two_model_enabled, default_model)` — pure, phase supplied by the call site (no message-history introspection); the single-model default is owned once by `llm_router.DEFAULT_MODEL` / `configured_default_model()` (read llm-free so injected-client turns never import the Anthropic SDK). The per-org gate is `organizations.agent_two_model_enabled` (BOOLEAN, default `false`; migration `supabase/046_organizations_agent_two_model.sql`), resolved fail-safe-to-off by `llm_router.resolve_org_two_model_enabled` via `to_jsonb(o)->>'…'` (same shape as the budget knob — **no env var**, named off "sql" because the split is on the consumption path, not SQL mode). `src/api/agent/controller.py` resolves the flag once per turn, threads it through the `LLMClient`, and records the chosen `{explore_model, format_model}` in `agent_runs.steps_json` (the `model_route` step) so the routing is auditable. Cost gate: `tests/golden/test_agent_golden.py::test_two_model_cost_reduction_vs_single` (deterministic price-table × token estimate) shows ≈46% lower cost on the 52-question suite — the saving comes entirely from sending the large static extraction prompt to a ~3× cheaper model. **The split is OFF by default and has NOT been rolled out**; Haiku's extraction *accuracy* is unverified until a live `--golden` run (the `two_model` variant of `test_extract_plan_golden`) clears the ≥48/52 target.

**Rollout procedure (one org at a time):**
1. **Enable a pilot org** — `UPDATE organizations SET agent_two_model_enabled = true WHERE id = '<pilot-org-uuid>';` (the only switch; no redeploy).
2. **Monitor one week** — three must all hold (note: these are the *consumption-path* signals; the SQL-mode taxonomy `validator_rejected` / `empty_result` and the S3.5 shadow suite **do not apply**, since the split is not on the SQL loop):
   - **Accuracy** — a live `test_extract_plan_golden` run still clears ≥48/52 on the `two_model` variant (Haiku extraction), and the agent's extraction-error / overall error rate does not rise.
   - **Cost** — per-turn LLM spend for the org drops (watch `favonius_agent_llm_tokens_total` split by `model`) by at least the golden-suite delta.
   - **No quality regressions** in the consumption answers the pilot org sees.
3. **All three hold →** broaden org-by-org, ultimately org-wide. (Do this deliberately; there is no global flag to flip.)
4. **Any fails →** `UPDATE organizations SET agent_two_model_enabled = false WHERE id = '<pilot-org-uuid>';` and open a follow-up issue with the regression.
...
- **Approve → recurring schedule (frontend-owned):** `agents.action.approve` on a **one-off** `report_draft` (payload has no `scheduleId`) generates the report (draft) and flips the action to `executed` — it does **not** create a schedule. The recurring monthly schedule is created by the **frontend**, which fires a separate `reports.schedule.create` right after the approve succeeds (`use-agents.ts`): `monthly` on day 1 at 06:00 depot-local, `autonomyMode=auto_notify`, recipient = the approving user's email, format `pdf`. The frontend owns the idempotency check ("an active `monthly_consumption` schedule already exists → skip") so the backend deliberately stays out of it to avoid double-creation. (A schedule-originated draft, with `scheduleId`+`runId`, instead just delivers the existing report — that path is unchanged.) **Note:** `reports.schedule.create` requires `ADMIN_CONFIG` (customer_admin+) while `agents.action.approve` only requires `DEPOT_MANAGE` (operator+), so a customer_operator's approve succeeds but the follow-on schedule-create 403s — surface this to product if operators need the recurring schedule. Separately, `reports.approve` on a standalone `monthly_consumption` report emails the rendered PDF to the **approving user** (`_deliver_report_to_approver`, best-effort, post-commit) and surfaces the outcome under `result.delivery`.
...
### Context-aware support summary (`src/observability/` + `src/api/main.py`)

The "This isn't right" button: a depot user reports a problem and the backend bundles the moment's context for engineering. The frontend captures the screen and `POST`s JSON to `/support/summary` (`{page, user_note?, depot_id?, screenshot_base64?, screenshot_content_type?}` — base64, not multipart, so no `python-multipart` dependency); the backend gathers (1) the screenshot, (2) the last ~5 min of API logs, and (3) a live health snapshot of **Tiger Cloud (= TimescaleDB, `db_pools.ts`)** + the **WebSocket/OCPP server**, then renders the exact sentence `build_summary_text(...)` produces: `User {id} reported an error while viewing {page}. System status at time of report: Tiger Cloud {status}, WebSocket {status}. Last error log: {error}.` and persists it to `support_summaries` (migration 047, TS pool, BYTEA screenshot). Delivery to engineering is post-commit best-effort via the existing `report_email_client` (`SUPPORT_ENG_RECIPIENT`); the row is the source of truth.

Logs come from a new in-process **ring-buffer log handler** (`src/observability/log_buffer.py::RingBufferLogHandler`, installed on the root logger in the lifespan, bounded by count **and** age — there was no log store before this). Known limitation: per-process, lost on restart, and under multiple workers only the worker that served the request has its logs (a durable `app_logs` table is the documented upgrade path). Health helpers live in `main.py`: `check_tiger_cloud_health` pings the **`ts`** pool (the existing `check_database_health` only covers Supabase/`static`); `check_websocket_health` uses the in-process `ocpp_server` when enabled, else an HTTP probe to the legacy handler's `:8081/health` (`WEBSOCKET_HEALTH_PROBE_URL`), else `"unknown"`. The orchestration is in `src/observability/support_summary.py` (builder, `decode_screenshot`, redacted/bounded log excerpt, persist/fetch/delete/purge/deliver/erase).

**GDPR:** lawful basis = legitimate interest (user-initiated diagnostics); data-minimised to `user_id` (UUID, never email); `user_note`/logs/error line pass through `log_buffer.redact_log_line` (emails, bearer/JWT, secrets, long hex); retention-bounded via `expires_at` (`SUPPORT_SUMMARY_RETENTION_DAYS`, default 90) and hard-purged by `run_support_summary_purge_loop`; right-to-erasure via the admin `DELETE` endpoint + `erase_support_summaries_for_user` (DSAR hook); every create/read/delete writes an `audit_log` row (`support.summary.{created,read,deleted}`). Reads/erasure are `favonius_admin`-only; filing a report is open to any authenticated tenant user. Endpoints: `POST /support/summary` (any member), `GET /support/summary/{id}` + `/{id}/screenshot` + `DELETE /support/summary/{id}` (favonius_admin). Env knobs (`SUPPORT_LOG_BUFFER_*`, `SUPPORT_SCREENSHOT_MAX_BYTES`, `SUPPORT_SUMMARY_*`, `SUPPORT_ENG_RECIPIENT`, `WEBSOCKET_HEALTH_PROBE_URL`) are in `.env.example`. Tests: `tests/unit/observability/` + `tests/unit/test_api_support_summary.py`.
...
### Navirec telematics — live SoC feed (`src/adapters/navirec/` + migration 044)
...
**Data-graph wiring convention:** `GET /depots/{id}/data-graph` (`src/api/main.py::_build_data_graph`) returns live source-health nodes for the frontend Sources page. **Every new data source integration must be wired into `_build_data_graph`** so its status dot appears automatically. The function's docstring contains the exact 5-step wiring pattern; follow it for each new provider (env flag / DB row detection → health query → status mapping → `DataGraphNode` with `id = "src:{provider_key}"` → unit tests in `tests/unit/test_api_data_graph.py`).
...
| Organization | `organizations` | `id` (`AS organization_id`) | `name`, `type`, `billing_address`, `primary_contact`, `subscription_tier`, `is_active`, `agent_sql_mode_enabled` (BOOLEAN, default TRUE; gates chat SQL mode per org — mig `supabase/044`), `agent_two_model_enabled` (BOOLEAN, default FALSE; routes the consumption fast-path extraction to Haiku per org — mig `supabase/046`) |
...
- `vehicle_telemetry` — Telematics SoC/position from Navirec (migration 044). Second SoC source, merged with `telemetry` (freshest-wins) in `StateAssembler._get_vehicle_socs`. `time` is the device's own timestamp; 90-day retention; index `(vehicle_id, time DESC)`
...
- `support_summaries` — Context-aware support reports ("This isn't right"): screenshot (BYTEA) + redacted log excerpt + health snapshot + engineering summary (migration 047). GDPR-retention-bounded via `expires_at` + purge loop. See "Context-aware support summary".
...
| `SOLVER_PROCESS_POOL_SIZE` | `1` | Worker processes for MILP solves. Solves run out of the API process so the asyncio loop stays responsive. Each persistent worker holds Pyomo+Gurobi resident (~0.3–0.5 GB); raise only when depots must solve concurrently. |
| `SOLVER_WORKER_MAX_TASKS` | `20` | Recycle each solver worker after this many solves (`max_tasks_per_child`) so Pyomo/Gurobi memory does not creep across solves. `<=0` disables recycling (workers live for the pool's lifetime). |
| `SOLVER_WORKER_AS_LIMIT_MB` | `1500` | Per-worker `RLIMIT_AS` ceiling. A runaway solve crashes the worker, not the API container. Lower (e.g. 1000) to cap spikes on a small-fleet deploy. |
...
- Each MILP solve runs in a child process via `ProcessPoolExecutor` (spawn mode), so the FastAPI event loop stays responsive during a 60 s solve. Default 1 worker (`SOLVER_PROCESS_POOL_SIZE`); per-worker virtual-memory ceiling via `RLIMIT_AS` (`SOLVER_WORKER_AS_LIMIT_MB`, default 1500 MiB). Each worker is recycled after `SOLVER_WORKER_MAX_TASKS` solves (default 20, `max_tasks_per_child`) so Pyomo/Gurobi memory is returned to the OS rather than accumulating across solves.
