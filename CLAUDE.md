...
- **Reverse-direction self-heal:** `repair_user_tenant_metadata` runs first in `ensure_tenant_mirrored`. When a JWT lacks `app_metadata.organization_id` but the user has exactly one `user_organizations` row whose role is `owner` or `operator`, the repair `PUT`s the derived claims (`favonius_role`, `organization_id`, `organization_name`) to `{SUPABASE_URL}/auth/v1/admin/users/{user_id}` using `SUPABASE_SERVICE_KEY`. The current request still proceeds without org context — the user's *next* token refresh sees the corrected claims and the first-depot wizard becomes visible. Skipped silently if `SUPABASE_URL`/`SUPABASE_SERVICE_KEY` are unset, the user has zero or multiple memberships, or the membership role is `admin`/`viewer` (the `admin` exclusion prevents a corrupted DB row from escalating to `favonius_admin`). Successful repairs log `tenant_metadata_repair: backfilled app_metadata` at WARNING level so high counts surface a frontend-signup regression.
