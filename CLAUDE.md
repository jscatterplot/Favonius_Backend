...
Price lookup goes through `src/db/queries.py::fetch_prices_by_zone` (reads `electricity_prices` keyed by ENTSO-E `node_id`, converts €/MWh → €/kWh). Each call site resolves the ENTSO-E bidding zone for a depot via `src/db/queries.py::resolve_bidding_zone(static_pool, site_id)` once, with a cascade: `sites.tariff_config['entsoe_zone']` → `get_bidding_zone(sites.timezone)` → `None`. Callers (cost calculator + `StateAssembler._get_prices`) inject the resolved zone before reading prices; the calculator returns `'unpriceable'` and the optimizer falls back to $0.15/kWh when no zone resolves. The legacy `prices` table (per-depot tariff) is unused on the current ENTSO-E deployment.
