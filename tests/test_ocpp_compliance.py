...
from websocket_handler.device_model import DeviceModel, Component, DeviceVariable, VariableAttribute, VariableType
from websocket_handler.charging_profile_manager import ChargingProfileManager, ChargingProfile, ChargingSchedule, ChargingSchedulePeriod, ChargingProfilePurpose, ChargingProfileKind, ChargingRateUnit
from websocket_handler.transaction_manager import TransactionManager, IdToken, IdTokenType, TransactionInfo, ChargingState
from websocket_handler.certificate_manager import CertificateManager, CertificateType, CertificateInfo, CertificateStatus
...
class TestDeviceModel:
    ...
    async def test_get_variables_success(self, device_model, mock_timescale_client):
        ...
        mock_timescale_client.get_device_variable.return_value = {
            "value": "TestValue"
        }
...
class TestChargingProfileManager:
    ...
    @pytest.mark.asyncio
    async def test_set_charging_profile_success(self, charging_profile_manager, sample_charging_profile):
    ...
    async def test_get_composite_schedule_no_profiles(self, charging_profile_manager):
        ...
        result = await charging_profile_manager.get_composite_schedule(
            "test_station", 1, 3600
        )
    ...
    @pytest.mark.asyncio
    async def test_get_composite_schedule_with_profiles(self, charging_profile_manager, mock_timescale_client):
...
class TestTransactionManager:
    ...
    @pytest.mark.asyncio
    async def test_request_start_transaction_success(self, transaction_manager, sample_id_token, mock_timescale_client):
    ...
    @pytest.mark.asyncio
    async def test_request_start_transaction_evse_unavailable(self, transaction_manager, sample_id_token, mock_timescale_client):
    ...
    @pytest.mark.asyncio
    async def test_request_stop_transaction_success(self, transaction_manager, mock_timescale_client):
    ...
    @pytest.mark.asyncio
    async def test_request_stop_transaction_not_found(self, transaction_manager, mock_timescale_client):
...
class TestCertificateManager:
    ...
    async def test_delete_certificate_no_match(self, certificate_manager):
        ...
        result = await certificate_manager.delete_certificate(
            "test_station", certificate_hash_data
        )
...
class TestOCPPIntegration:
    ...
    async def test_boot_notification_flow(self, mock_timescale_client):
        ...
        charge_point = EnhancedOCPPChargePoint(
            "test_station", mock_connection, mock_config, 
            mock_timescale_client, mock_connection_manager
        )
