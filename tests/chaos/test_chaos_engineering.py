...
from src.websocket_handler.enhanced_error_handler import (
...
class TestResourceExhaustionResilience:
    ...
    async def test_memory_pressure_resilience(self, der_control_manager, chaos_injector):
        ...
        try:
            ...
            assert (
                success_rate > 0.3
            ), f"Success rate too low under memory pressure: {success_rate:.1%}"
...
class TestNetworkFailureResilience:
    ...
    async def test_network_latency_resilience(self, der_control_manager, chaos_injector):
        ...
        delayed_operation = chaos_injector.inject_network_latency(base_latency=0.5, jitter=0.2)
        ...
        der_control_manager.timescale_client.execute_query = (
            lambda *args, **kwargs: delayed_operation(
        ...
        try:
            ...
            ), f"Response time too low (latency not applied): {avg_response_time:.3f}s"
...
class TestCascadingFailureResilience:
    ...
    async def test_cascading_database_failures(self, der_control_manager_with_failures):
        ...
        for failure_rate in failure_rates:
            ...
            der_control_manager_with_failures.timescale_client.execute_query = failure_methods[
                "execute_query"
            ]
            der_control_manager_with_failures.timescale_client.fetch_one = failure_methods[
                "fetch_one"
            ]
            der_control_manager_with_failures.timescale_client.fetch_all = failure_methods[
                "fetch_all"
            ]
...
class TestRecoveryResilience:
    ...
    async def test_system_recovery_after_failures(self, der_control_manager, chaos_injector):
        ...
        for i in range(total_operations):
            ...
            try:
                result = await der_control_manager.set_der_control(f"station_{i % 5}", control_data)
                if result["status"] == "Accepted":
        ...
        ), f"Recovery success rate too low: {recovery_success_rate:.1%}"
