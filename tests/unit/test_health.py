...
class TestHealthCheckServer:
    ...
    async def test_health_check_endpoint(self, health_server):
        ...
        with patch("src.websocket_handler.health.health_checker") as mock_health_checker:
            mock_health_checker.run_checks = AsyncMock(
                return_value={
                    "status": "healthy",
    ...
    async def test_status_endpoint(self, health_server):
        ...
        with (
            patch("src.websocket_handler.health.health_checker") as mock_health_checker,
            ...
            mock_health_checker.get_last_results = Mock(
                return_value={"last_check": "2024-01-01T12:00:00Z", "status": "healthy"}
            )

            mock_metrics_collector.get_summary_stats = Mock(
                return_value={"active_connections": 5, "total_requests": 1000, "error_rate": 0.01}
            )
    ...
    async def test_metrics_summary_endpoint(self, health_server):
        ...
        with patch("src.websocket_handler.health.metrics_collector") as mock_metrics:
            mock_metrics.get_summary_stats = Mock(
                return_value={
                    "websocket_connections": {"active": 5, "total": 100, "peak": 20},
                    "api_requests": {
                        "total": 1000,
                        "successful": 950,
                        "failed": 50,
