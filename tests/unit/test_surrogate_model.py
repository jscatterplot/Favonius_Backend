...
def test_get_r2_score_on_validation(model, synthetic_training_data, synthetic_validation_data):
...
@pytest.mark.slow
def test_get_r2_score_meets_requirement(model, synthetic_training_data, synthetic_validation_data):
...
def test_save_load_preserves_predictions(model, synthetic_training_data, tmp_path):
...
def test_full_training_pipeline(model, synthetic_training_data, synthetic_validation_data):
...
class TestSurrogateModelRobustness:
    ...
    def test_all_same_route(self, known_routes):
        ...
        # All same route
        inputs = [
            generate_synthetic_input(route_id="route_1", temp_avg_f=50.0 + i) for i in range(30)
        ]
