...
from src.websocket_handler.auth_manager import (
    AuthManager,
    RateLimiter,
    rate_limit,
    require_auth,
...
class TestAuthManager:
    ...
    async def test_authenticate_user_valid_token(self, auth_manager):
        ...
        token = jwt.encode(
            token_payload, auth_manager.jwt_secret, algorithm=auth_manager.jwt_algorithm
        )
        ...
        with patch.object(
            auth_manager, "_get_user_from_supabase", new_callable=AsyncMock, return_value=user_data
        ):
    ...
    async def test_get_user_from_supabase(self, auth_manager):
        ...
        auth_manager.supabase_client.client.table.return_value.select.return_value.eq.return_value.execute.return_value = (
            mock_response
        )
    ...
    async def test_authorize_action_denied(self, auth_manager):
        ...
        user = {"id": "user123", "role": "user", "organization_id": "org1", "permissions": ["read"]}
    ...
    def test_generate_token(self, auth_manager):
        ...
        payload = jwt.decode(
            token, auth_manager.jwt_secret, algorithms=[auth_manager.jwt_algorithm]
        )
    ...
    def test_verify_api_key_valid(self, auth_manager):
        ...
        api_key = jwt.encode(
            api_key_payload, auth_manager.jwt_secret, algorithm=auth_manager.jwt_algorithm
        )
...
class TestAuthDecorators:
    ...
    async def test_require_permission_decorator_success(self, mock_auth_manager):
        ...
        with patch.object(
            AuthManager, "authorize_action", new_callable=AsyncMock
        ) as mock_authorize:
