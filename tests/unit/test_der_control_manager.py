...
from src.websocket_handler.der_control_manager import (
    DERControlManager, DERControlEnumType, DERCurveType, DERCurvePoint,
    DERControlType, DERCurve
)
...
class TestDERCurve:
    ...
    def test_curve_creation(self):
        ...
        curve = DERCurve(
            curve_type=DERCurveType.VOLT_VAR,
            points=points,
            curve_unit_x="V",
            curve_unit_y="VAR"
        )
...
class TestDERControlType:
    ...
    def test_der_control_type_creation(self):
        ...
        control_type = DERControlType(
            control_id=1,
            control_type=DERControlEnumType.VOLT_VAR,
            priority=1
        )
...
class TestDERControlManager:
    ...
    async def test_set_der_control(self, der_control_manager):
        ...
        control_data = {
            "control_type": "VoltVar",
            "priority": 1
        }
        
        with patch.object(der_control_manager, '_parse_der_control') as mock_parse, \
             patch.object(der_control_manager, '_validate_der_control', return_value={"valid": True}), \
            ...
            mock_control = DERControlType(control_id=1, control_type=DERControlEnumType.VOLT_VAR, priority=1)
    ...
    async def test_set_der_control_invalid(self, der_control_manager):
        ...
        with patch.object(der_control_manager, '_parse_der_control') as mock_parse, \
             patch.object(der_control_manager, '_validate_der_control', return_value={"valid": False, "reason_code": "InvalidType", "message": "Invalid control type"}):
            
            mock_control = DERControlType(control_id=1, control_type=DERControlEnumType.FIXED_PF_INJECT, priority=1)
            mock_parse.return_value = mock_control
