...
class TestStorePrices:
    ...
    @pytest.mark.asyncio
    async def test_store_prices_success(self, mock_db_pool, depot_id, sample_prices):
    ...
    @pytest.mark.asyncio
    async def test_store_prices_converts_lmp_to_kwh(self, mock_db_pool, depot_id):
    ...
    @pytest.mark.asyncio
    async def test_store_prices_with_demand_charge(self, mock_db_pool, depot_id, sample_prices):
        """Test storage with demand charge rate."""
        pool, conn = mock_db_pool

        demand_charge = 20.0  # $/kW

        await store_prices(pool, sample_prices[:1], depot_id, demand_charge_per_kw=demand_charge)
    ...
    @pytest.mark.asyncio
    async def test_store_prices_with_custom_source(self, mock_db_pool, depot_id, sample_prices):
    ...
    @pytest.mark.asyncio
    async def test_store_prices_handles_db_error(self, mock_db_pool, depot_id, sample_prices):
    ...
    @pytest.mark.asyncio
    async def test_store_prices_with_uuid_depot_id(self, mock_db_pool, sample_prices):
...
class TestDuplicateHandling:
    ...
    @pytest.mark.asyncio
    async def test_store_duplicate_timestamps(self, mock_db_pool, depot_id):
...
class TestGapFillingLogic:
    ...
    async def test_identify_price_gaps(self, mock_db_pool, depot_id):
        ...
        prices = await get_cached_prices(pool, depot_id, base_time, base_time + timedelta(hours=4))
...
class TestTimestampHandling:
    ...
    async def test_retrieve_ordered_by_time(self, mock_db_pool, depot_id):
        ...
        await get_cached_prices(pool, depot_id, datetime(2025, 1, 1), datetime(2025, 1, 2))
