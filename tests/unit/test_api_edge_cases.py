    ...
    async def test_concurrent_optimize_requests(
        ...
        with patch('src.api.main.db_pool', pool):
            with patch('src.api.main.controller_manager', mock_controller_manager):
                with patch('src.api.main._get_depot_config', AsyncMock(return_value=sample_depot_config)):
            ...
            with patch('src.api.main._get_depot_config', AsyncMock(return_value=sample_depot_config)):
                with patch('src.api.main.StateAssembler', return_value=mock_assembler):
                    async with AsyncClient(
                        ...
                        depot_id = str(uuid4())
                        tasks = [
                            ac.get(f"/depots/{depot_id}/state")
                            for _ in range(10)
                        ]
                        responses = await asyncio.gather(*tasks)
...
class TestTimeoutHandling:
    ...
    async def test_optimize_timeout(
        self, mock_db_pool, sample_depot_config
    ):
...
class TestResponseSerialization:
    ...
    @pytest.mark.asyncio
    async def test_state_response_with_many_vehicles(
        self, mock_db_pool, sample_depot_config
    ):
