...
from src.websocket_handler.certificate_manager import (
    CertificateManager, CertificateType, CertificateStatus, CertificateInfo,
...
class TestCertificateManager:
    ...
    async def test_get_15118_ev_certificate_success(self, certificate_manager):
        ...
        with patch.object(certificate_manager, '_get_certificate', return_value=mock_certificate):
            result = await certificate_manager.get_15118_ev_certificate(station_id, certificate_type)
    ...
    async def test_get_15118_ev_certificate_exception(self, certificate_manager):
        ...
        with patch.object(certificate_manager, '_get_certificate', side_effect=Exception("Test error")):
            result = await certificate_manager.get_15118_ev_certificate(station_id, certificate_type)
    ...
    async def test_certificate_signed_success(self, certificate_manager):
        ...
        with patch.object(certificate_manager, '_validate_certificate_chain', return_value={"valid": True}), \
             patch.object(certificate_manager, '_parse_certificate', return_value=CertificateInfo(
                 certificate_type=certificate_type,
             ...
             patch.object(certificate_manager, '_install_certificate', return_value={"status": "Accepted"}):
    ...
    async def test_certificate_signed_invalid_chain(self, certificate_manager):
        ...
        with patch.object(certificate_manager, '_validate_certificate_chain', return_value={"valid": False, "reason_code": "InvalidCertificateChain", "message": "Invalid chain"}):
            result = await certificate_manager.certificate_signed(
                station_id, certificate_type, certificate_chain, exi_response
            )
    ...
    async def test_certificate_signed_storage_failure(self, certificate_manager):
        ...
        with patch.object(certificate_manager, '_validate_certificate_chain', return_value={"valid": True}), \
             ...
             patch.object(certificate_manager, '_check_certificate_limits', return_value=True), \
             patch.object(certificate_manager, '_install_certificate', return_value={"status": "Rejected", "statusInfo": {"reasonCode": "InternalError", "additionalInfo": "Failed to store certificate"}}):
    ...
    async def test_delete_certificate_success(self, certificate_manager):
        ...
        with patch.object(certificate_manager, '_find_certificate_by_hash', return_value="cert123"), \
    ...
    async def test_delete_certificate_exception(self, certificate_manager):
        ...
        with patch.object(certificate_manager, '_find_certificate_by_hash', side_effect=Exception("Test error")):
            result = await certificate_manager.delete_certificate(station_id, certificate_hash_data)
    ...
    def test_certificate_installation_result_creation(self):
        ...
        result = CertificateInstallationResult(
            status="Accepted",
            status_info={"reasonCode": "NoError"}
        )
