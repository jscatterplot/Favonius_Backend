...
class TestAnalyticsService:
    ...
    async def test_initialize(self, analytics_service, mock_timescale_client):
        ...
        with patch(
            "src.websocket_handler.analytics_service.TimescaleClient",
            return_value=mock_timescale_client,
        ):
    ...
    async def test_get_performance_analytics_basic(self, analytics_service, mock_timescale_client):
        ...
        with (
            patch.object(
                analytics_service, "_calculate_uptime_metrics", return_value={"uptime": 0.95}
            ),
            patch.object(
                analytics_service,
                "_calculate_efficiency_metrics",
                return_value={"efficiency": 0.88},
            ),
            patch.object(
                analytics_service,
                "_calculate_reliability_metrics",
                return_value={"reliability": 0.92},
            ),
        ):
