...
class TestPriceFeederService:
    ...
    async def test_trigger_fetch_error(self, price_feeder):
        ...
        with patch.object(price_feeder, '_fetch_and_store_prices', side_effect=Exception("Fetch error")):
    ...
    @pytest.mark.asyncio
    async def test_fetch_and_store_prices_success(self, price_feeder, mock_timescale_client, mock_optimization_engine):
