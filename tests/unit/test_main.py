...
from src.websocket_handler.main import Application, run_application, main
...
class TestApplication:
    ...
    async def test_initialize_supabase_components(self, mock_config):
        ...
        with patch('src.websocket_handler.main.SupabaseClient', return_value=mock_supabase_client), \
             patch('src.websocket_handler.main.AuthManager', return_value=mock_auth_manager), \
             patch('src.websocket_handler.main.DataSyncService', return_value=mock_data_sync_service), \
    ...
    async def test_initialize_timescale_components(self, mock_config):
        ...
        with patch('src.websocket_handler.main.TimescaleClient', return_value=mock_timescale_client), \
             patch('src.websocket_handler.main.AnalyticsService', return_value=mock_analytics_service), \
             patch('src.websocket_handler.main.OptimizationEngine', return_value=mock_optimization_engine), \
    ...
    async def test_start_application_success(self, mock_config):
        ...
        with patch.object(app, '_initialize_supabase_components', AsyncMock()), \
             patch.object(app, '_initialize_timescale_components', AsyncMock()), \
             patch('src.websocket_handler.main.setup_monitoring'), \
             patch('src.websocket_handler.main.OCPPWebSocketServer', return_value=mock_websocket_server), \
             ...
             patch('src.websocket_handler.main.ConnectionMonitor', return_value=mock_connection_monitor), \
             patch.object(app, 'data_sync_service', mock_data_sync_service):
            
            # Mock the websocket server start to complete immediately
            mock_websocket_server.start = AsyncMock(side_effect=asyncio.CancelledError("Test completion"))
...
class TestRunApplication:
    ...
    async def test_run_application_success(self):
        ...
        with patch('src.websocket_handler.main.Application', return_value=mock_app), \
...
class TestMainFunction:
    ...
    def test_main_function_success(self):
        ...
        with patch('src.websocket_handler.main.Config.from_env') as mock_from_env, \
