...
class TestDataSyncService:
    ...
    async def test_get_sync_status(self, data_sync_service):
        ...
        data_sync_service.sync_running = True
        data_sync_service.last_sync_times = {
            'charging_sessions': datetime.now(timezone.utc)
        }
    ...
    async def test_force_sync(self, data_sync_service):
        ...
        with patch.object(data_sync_service, 'sync_charging_sessions', return_value=None) as mock_sessions, \
             patch.object(data_sync_service, 'sync_energy_metrics', return_value=None) as mock_energy, \
             patch.object(data_sync_service, 'sync_vehicle_states', return_value=None) as mock_states:
