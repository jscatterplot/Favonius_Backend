...
from src.core.optimizer import InfeasibleModelError, InvalidStateError
...
class TestOptimizerErrorPropagation:
    ...
    @pytest.mark.asyncio
    async def test_optimizer_timeout_recorded(
        self, mock_db_pool, depot_config, controller_config
    ):
...
class TestOCPPErrorPropagation:
    ...
    @pytest.mark.asyncio
    async def test_ocpp_timeout_handled(
        self, mock_db_pool, depot_config, controller_config
    ):
        ...
        mock_cp.set_charging_profile = AsyncMock(
            side_effect=asyncio.TimeoutError("OCPP timeout")
        )
