...
class TestOCPPIntegration:
    ...
    async def ocpp_handler(self, timescale_client):
        ...
        return EnhancedOCPPChargePoint(
            "TEST_STATION_001", mock_connection, config, client, connection_manager
        )
    ...
    async def test_complete_charging_session_flow(self, ocpp_handler, timescale_client):
        ...
        await timescale_client.__anext__()
        ...
        boot_result = handler.on_boot_notification(charging_station, "PowerUp")
        ...
        start_result = await ocpp_handler.on_request_start_transaction(
            evse_id=1, id_token=id_token, remote_start_id=1
        )
        ...
        from ocpp.v21.enums import MeasurandEnumType, ReadingContextEnumType, UnitOfMeasureEnumType
        ...
        meter_result = await ocpp_handler.on_meter_values(
            evse_id=1, meter_value=[meter_value], transaction_id=transaction_id
        )
        ...
        transaction_ended = await ocpp_handler.on_transaction_event(
            ...
            seq_no=2,
            transaction_info={"transactionId": transaction_id, "chargingState": "EVDisconnected"},
            id_token=id_token,
        )
    ...
    async def test_charging_profile_management_flow(self, ocpp_handler, timescale_client):
        ...
        schedule_period = ChargingSchedulePeriodType(start_period=0, limit=22.0)
        ...
        get_profiles_result = await ocpp_handler.on_get_charging_profiles(
            request_id=1, evse_id=1, charging_profile_purpose="TxDefaultProfile", stack_level=0
        )
        ...
        composite_result = await ocpp_handler.on_get_composite_schedule(
            request_id=2, evse_id=1, duration=3600, charging_rate_unit="W"
        )
        assert composite_result["status"] == "Accepted"

        # 4. Clear Charging Profile
        clear_result = await ocpp_handler.on_clear_charging_profile(
            charging_profile_id=1, charging_profile_purpose="TxDefaultProfile", evse_id=1
        )
    ...
    async def test_device_configuration_flow(self, ocpp_handler, timescale_client):
        ...
        base_report_result = await ocpp_handler.on_get_base_report(
            request_id=1, report_base="ConfigurationInventory"
        )
    ...
    async def test_monitoring_and_alerting_flow(self, ocpp_handler, timescale_client):
        ...
        monitoring_report_result = await ocpp_handler.on_get_monitoring_report(
            request_id=1, monitoring_criteria=[], component_variable=[]
        )
        ...
        notify_monitoring_result = await ocpp_handler.on_notify_monitoring_report(
            request_id=1, generated_at="2023-01-01T00:00:00Z", seq_no=1, monitoring_data=[]
        )
    ...
    async def test_display_message_flow(self, ocpp_handler, timescale_client):
        ...
        from ocpp.v21.datatypes import MessageContentType, MessageInfoType
        from ocpp.v21.enums import (
        ...
        message_content = MessageContentType(
            format=MessageFormatEnumType.utf8, language="en", content="Charging in progress"
        )
        ...
        set_message_result = await ocpp_handler.on_set_display_message(message=message_info)
        assert set_message_result["status"] == "Accepted"

        # 2. Clear Display Message
        clear_message_result = await ocpp_handler.on_clear_display_message(message_id=1)
    ...
    async def test_privacy_compliance_flow(self, ocpp_handler, timescale_client):
        ...
        from ocpp.v21.datatypes import IdTokenType
        from ocpp.v21.enums import CustomerInformationStatusEnumType, IdTokenEnumType
