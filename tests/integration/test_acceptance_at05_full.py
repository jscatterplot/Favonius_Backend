...
class TestAT05FullInterDepotHandoff:
    ...
    @pytest.mark.asyncio
    async def test_at05_handoff_message_within_30_seconds(
        self, mock_db_pool
    ):
    ...
    @pytest.mark.asyncio
    async def test_at05_depot_b_receives_handoff(
        self, mock_db_pool
    ):
    ...
    async def test_at05_full_multi_depot_flow(
        self, mock_db_pool, depot_a_config, depot_b_config, controller_config,
        depot_a_state, depot_b_state_before_handoff
    ):
        ...
        await conn.execute(
            ...
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            """,
            message_id, depot_a_id, depot_b_id, bus_1_id,
            departure_time, expected_soc, arrival_time,
        )
        ...
        controller_b.assembler.get_current_state = AsyncMock(
            return_value=depot_b_state_with_bus_1
        )
    ...
    @pytest.mark.asyncio
    async def test_at05_multiple_handoffs(
        self, mock_db_pool
    ):
        ...
        for bus_id in buses:
            ...
            await conn.execute(
                ...
                VALUES ($1, $2, $3, $4, $5, $6, $7)
                """,
                message_id, depot_a_id, depot_b_id, bus_id,
                datetime.utcnow(), 0.35, datetime.utcnow() + timedelta(hours=2),
            )
    ...
    @pytest.mark.asyncio
    async def test_at05_handoff_acknowledgment(
        self, mock_db_pool
    ):
    ...
    @pytest.mark.asyncio
    async def test_at05_handoff_with_low_soc(
        self, depot_b_config, depot_b_state_before_handoff
    ):
    ...
    async def test_at05_handoff_timing_constraints(
        ...
        state_tight_timing = DepotState(
            ...
            current_month_peak=depot_b_state_before_handoff.current_month_peak,
            vehicle_availability={
                'bus_1': [False] * arrival_timestep + [True] * (departure_timestep - arrival_timestep) + [False] * (n_t - departure_timestep),
                'bus_3': [True] * n_t,
                'bus_4': [True] * n_t,
            },
            energy_requirements={
