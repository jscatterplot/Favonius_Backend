...
class TestHourlyOptimizationTrigger:
    ...
    async def test_hourly_trigger_fires_on_schedule(
        ...
        async def run_briefly():
            ...
            now = datetime.utcnow()
            if (
                fast_controller_config.hourly_optimization_start
                <= now.hour
                <= fast_controller_config.hourly_optimization_end
            ):
                if controller.last_run_time is None or (now - controller.last_run_time) > timedelta(
                    hours=1
                ):
    ...
    async def test_hourly_trigger_respects_time_window(
        self, mock_db_pool, sample_depot_config, sample_depot_state, sample_optimization_result
    ):
...
class TestPriceSpikeTrigger:
    ...
    async def test_price_spike_shifts_charging(
        ...
        with patch("src.core.controller.optimize") as mock_optimize:
            ...
            spike_period_power = sum(result.schedule["bus_1"]["charging_power"][32:64])
            non_spike_power = sum(result.schedule["bus_1"]["charging_power"][:32]) + sum(
...
class TestFullIntegrationFlow:
    ...
    async def test_trigger_reoptimization_updates_schedule(
        self, mock_db_pool, sample_depot_config, sample_depot_state, fast_controller_config
    ):
