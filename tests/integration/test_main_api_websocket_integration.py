...
def mock_websocket_handler_internal_api():
    ...
    api.get_charge_points = AsyncMock(return_value=[
        {
    ...
    api.get_charge_point_state = AsyncMock(return_value={
        "station_id": "charger_001",
        "connected": True,
        "evse_id": 1,
        "connector_id": 1,
    ...
    # Command dispatch
    api.send_charging_profile = AsyncMock(return_value={
        "success": True,
        "message": "Charging profile set successfully",
    })
    
    # Health check
    api.health_check = AsyncMock(return_value={
        "status": "healthy",
...
class TestMainAPIToWebSocketHandler:
    ...
    @pytest.mark.asyncio
    async def test_query_connected_charge_points(
    ...
    @pytest.mark.asyncio
    async def test_query_charge_point_state(
        self, mock_ocpp_client, mock_db_pool
    ):
    ...
    async def test_send_charging_profile_via_websocket_handler(
        ...
        result = await mock_ocpp_client.send_charging_profile(
            "charger_001", profile
        )
...
class TestTelemetryFlow:
    ...
    @pytest.mark.asyncio
    async def test_telemetry_stored_to_timescale(
        self, mock_db_pool, mock_timescale_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_telemetry_forwarded_to_main_api(
        self, mock_ocpp_client, mock_db_pool
    ):
...
class TestCommandDispatchFlow:
    ...
    async def test_optimization_result_dispatched_via_websocket_handler(
        self, mock_ocpp_client, mock_db_pool, sample_depot_config,
        fast_controller_config, sample_depot_state
    ):
        ...
        with patch('src.core.controller.optimize', return_value=result), \
             patch.object(StateAssembler, 'load_depot_config', new_callable=AsyncMock) as mock_load_dispatch:
    ...
    @pytest.mark.asyncio
    async def test_command_dispatch_failure_handling(
        self, mock_ocpp_client, mock_db_pool
    ):
...
class TestBackupHeuristicActivation:
    ...
    async def test_backup_heuristic_activates_after_one_hour(
        ...
        mock_engine._check_main_api_health = AsyncMock(side_effect=lambda: main_api_health_checks.pop(0) if main_api_health_checks else True)
...
class TestEndToEndIntegration:
    ...
    async def test_full_flow_charger_to_optimization_to_command(
        self, mock_ocpp_client, mock_db_pool, mock_timescale_client,
        sample_depot_config, fast_controller_config, sample_depot_state
    ):
