...
def realistic_depot():
    ...
    state = DepotState(
        vehicle_socs={f"bus_{i}": 0.7 + (i % 5) * 0.03 for i in range(20)},
        battery_soc=0.5,
        ...
        departure_times={f"bus_{i}": 32 + (i * 2) for i in range(20)},
    ...
    for vid, t_dep in state.departure_times.items():
        if vid in result.schedule:
            ...
            assert soc_at_departure >= 0.99, (
                f"{vid} not charged: SoC={soc_at_departure:.3f} " f"at timestep {t_dep}"
            )
...
def test_charger_capacity_constraint(realistic_depot):
    ...
    for t in range(config.n_timesteps):
        ...
        charging_count = sum(
            1 for vid in result.schedule.keys() if result.schedule[vid]["charging_power"][t] > 0.1
        )

        total_chargers = sum(config.charger_groups.values())
        assert charging_count <= total_chargers, (
            f"Too many vehicles charging at timestep {t}: " f"{charging_count} > {total_chargers}"
        )
...
def test_warm_start_performance(realistic_depot):
    ...
    assert warm_start_time <= cold_start_time * 1.2, (
        f"Warm-start not faster: cold={cold_start_time:.2f}s, " f"warm={warm_start_time:.2f}s"
    )
...
def test_trigger_based_reoptimization(realistic_depot):
    ...
    updated_state = DepotState(
        vehicle_socs={
            vid: soc - 0.1 if vid == "bus_0" else soc for vid, soc in state.vehicle_socs.items()
        },
        battery_soc=state.battery_soc,
        prices=state.prices,
    ...
    result2 = optimize(updated_state, config, time_limit=30.0, previous_result=result1)
    ...
    if "bus_0" in result2.schedule and "bus_0" in updated_state.departure_times:
        ...
        assert soc_at_departure >= 0.99, (
            f"bus_0 not charged after re-optimization: " f"SoC={soc_at_departure:.3f}"
        )
