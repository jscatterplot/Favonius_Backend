...
class TestAT05ReturnTimeDeviationHandling:
    ...
    def delayed_state(self, depot_config):
        ...
        return DepotState(
            vehicle_socs={'bus_0': 0.32, 'bus_1': 0.52, 'bus_2': 0.57},  # bus_0 slightly lower due to delay
            battery_soc=0.5,
            prices=prices,
            demand_charge_rate=20.0,
            current_month_peak=200.0,
    ...
    @pytest.mark.asyncio
    async def test_at05_return_time_deviation_detection(
    ...
    @pytest.mark.asyncio
    async def test_at05_no_trigger_within_threshold(
        self, initial_state, depot_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_at05_early_return_no_trigger(
        self, initial_state, depot_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_at05_multiple_vehicles_late(
        self, depot_config
    ):
