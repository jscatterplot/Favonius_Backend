...
class TestAT04SoCDeviationHandling:
    ...
    @pytest.mark.asyncio
    async def test_at04_soc_deviation_handling(self, initial_state, depot_config):
        ...
        assert soc_at_departure >= 0.98, f"bus_1 SoC at departure ({soc_at_departure:.3f}) < 0.98"
        ...
        sum(initial_result.schedule["bus_1"]["charging_power"][: check_timestep + 1]) / (
            check_timestep + 1
        )

        sum(reopt_result.schedule["bus_1"]["charging_power"][: check_timestep + 1]) / (
            check_timestep + 1
        )
