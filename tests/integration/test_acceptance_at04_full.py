...
class TestAT04FullSoCDeviationHandling:
    ...
    @pytest.mark.asyncio
    async def test_at04_soc_deviation_detected(self, depot_config, initial_state, trigger_config):
    ...
    async def test_at04_reoptimization_prioritizes_deviated_vehicle(
        ...
        bus_1_charging = sum(
            reopt_result.schedule["bus_1"]["charging_power"][t] for t in horizon_after_deviation
        )
        bus_2_charging = sum(
            reopt_result.schedule["bus_2"]["charging_power"][t] for t in horizon_after_deviation
        )
        bus_3_charging = sum(
            reopt_result.schedule["bus_3"]["charging_power"][t] for t in horizon_after_deviation
        )
    ...
    @pytest.mark.asyncio
    async def test_at04_departure_requirement_still_met(self, depot_config, initial_state):
    ...
    @pytest.mark.asyncio
    async def test_at04_multiple_vehicles_deviated(self, depot_config, initial_state):
    ...
    @pytest.mark.asyncio
    async def test_at04_deviation_with_constrained_charging(self, depot_config, initial_state):
    ...
    @pytest.mark.asyncio
    async def test_at04_severe_deviation_feasibility(self, depot_config, initial_state):
