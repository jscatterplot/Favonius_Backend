...
from websocket_handler.config import Config, PriceFeederConfig, OptimizationServiceConfig
...
class TestREVSDatasetValidation:
        ...
        for i in range(16):
            ...
            for j in range(100):  # 2.5 seconds of data
                ...
                data.append({
                    ...
                    "frequency_hz": 50.0 + (0.1 if j < 60 else 0.0),  # Frequency deviation during contingency
    ...
    @pytest.mark.asyncio
    async def test_revs_frequency_response_validation(self, mock_timescale_client, revs_sample_data):
        ...
        for data_point in contingency_data:
            if data_point["power_kw"] < 0:  # Discharge
                assert data_point["grid_frequency_hz"] > 50.0, "Should discharge when frequency is high"
...
class TestTUDortmundDatasetValidation:
            ...
            for i in range(count):
                ...
                for j in range(100):  # 100 time steps
                    ...
                    if "static" in profile_type:
                        ...
                        import math
                        power_kw = 22.0 * (1 + 0.5 * math.sin(j * 0.1)) if not bidirectional else 22.0 * math.sin(j * 0.1)
                    ...
                    data.append({
                        ...
                        "efficiency": 0.95,
                        "thd_percent": 2.0 + (0.5 if j % 2 == 0 else -0.5),  # Simulate THD variation
    ...
    @pytest.mark.asyncio
    async def test_tudortmund_harmonics_analysis(self, mock_timescale_client, tudortmund_sample_data):
    ...
    @pytest.mark.asyncio
    async def test_tudortmund_efficiency_analysis(self, mock_timescale_client, tudortmund_sample_data):
...
class TestElectricNationDatasetValidation:
    ...
    @pytest.mark.asyncio
    async def test_electric_nation_tariff_analysis(self, mock_timescale_client, electric_nation_sample_data):
    ...
    @pytest.mark.asyncio
    async def test_electric_nation_cost_calculation(self, mock_timescale_client, electric_nation_sample_data):
    ...
    @pytest.mark.asyncio
    async def test_electric_nation_v2g_revenue(self, mock_timescale_client, electric_nation_sample_data):
    ...
    @pytest.mark.asyncio
    async def test_electric_nation_peak_shaving(self, mock_timescale_client, electric_nation_sample_data):
...
class TestDatasetIntegration:
    ...
    async def test_dataset_validation_summary(self, mock_timescale_client):
        ...
        for dataset, results in validation_results.items():
            ...
            assert any(results[key] for key in results if isinstance(results[key], bool)), f"{dataset} should have validation results"
