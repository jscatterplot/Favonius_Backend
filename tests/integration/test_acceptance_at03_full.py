...
class TestAT03FullPriceSpikeReoptimization:
    ...
    async def test_at03_price_spike_triggers_within_60_seconds(
        ...
        now = datetime.utcnow()
        baseline_prices = {
            now + timedelta(hours=t * 0.25): price
    ...
    async def test_at03_full_controller_integration(
        ...
        async def capture_optimization(trigger_reason):
            ...
            current_state = (
                spiked_state if 'price' in trigger_reason.lower() else initial_state
            )
    ...
    @pytest.mark.asyncio
    async def test_at03_price_spike_threshold_boundary(
    ...
    @pytest.mark.asyncio
    async def test_at03_price_spike_below_threshold(
        self, depot_config, trigger_config
    ):
    ...
    async def test_at03_multiple_price_spikes(
        ...
        async def mock_optimization(trigger_reason):
            ...
            return OptimizationResult(
                run_id=uuid4(),
                schedule={v: {'charging_power': [0.0] * 96, 'soc': [0.5] * 96} 
    ...
    @pytest.mark.asyncio
    async def test_at03_solve_time_under_30_seconds(
        self, depot_config, spiked_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_at03_all_departures_satisfied(
        self, depot_config, spiked_state
    ):
