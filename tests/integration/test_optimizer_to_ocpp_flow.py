...
from src.adapters.ocpp.charge_point import convert_schedule_to_ocpp_profile
...
class TestOptimizationResultToOCPP:
    ...
    async def test_optimization_result_converted_to_ocpp_profile(self, optimization_result):
        ...
        assert all('numberPhases' in p for p in ocpp_profile), "Each period should have numberPhases"
    ...
    @pytest.mark.asyncio
    async def test_commands_dispatched_to_correct_chargers(
    ...
    async def test_command_dispatch_retry_logic(self, optimization_result):
        ...
        mock_charge_point.set_charging_profile = AsyncMock(
...
class TestEndToEndOptimizerToOCPPFlow:
    ...
    @pytest.mark.asyncio
    async def test_full_flow_state_to_ocpp_dispatch(
        self, depot_config, depot_state
    ):
    ...
    @pytest.mark.asyncio
    async def test_ocpp_dispatch_failure_retry_circuit_breaker(
        self, depot_config, depot_state
    ):
...
class TestServiceCommunication:
    ...
    async def test_main_api_to_websocket_handler_communication(self):
        ...
        success = await mock_ws_client.send_charging_profile(
            'charger_1', {'profile': 'data'}
        )
    ...
    async def test_telemetry_flow_ocpp_to_websocket_to_timescale(self):
        ...
        ocpp_meter_values = {
            ...
            'connector_id': 1,
            'meter_values': [
                {'timestamp': datetime.utcnow().isoformat(), 'energy_wh': 100000}
            ],
        }
        ...
        mock_timescale.get_latest_telemetry = AsyncMock(
            return_value={'soc': 0.5, 'power': 80.0}
        )
