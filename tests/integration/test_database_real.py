...
pytestmark = pytest.mark.skipif(
    not os.getenv("TEST_DATABASE_URL"), reason="TEST_DATABASE_URL environment variable not set"
)
...
class TestTelemetryWrites:
    ...
    async def test_concurrent_telemetry_writes(self, db_pool, test_vehicle_ids):
        ...
        tasks = [write_telemetry(vid, i) for i, vid in enumerate(test_vehicle_ids * 3)]
        results = await asyncio.gather(*tasks)
    ...
    async def test_store_charging_commands(self, db_pool, test_depot_id):
        ...
        async with db_pool.acquire() as conn:
            try:
                ...
                # Store command
                profile_json = json.dumps([{"start_period": 0, "limit": 80000, "number_phases": 3}])
...
class TestTransactions:
    ...
    async def test_concurrent_transactions(self, db_pool, test_depot_id):
        ...
        tasks = [run_transaction(db_pool, test_depot_id, 1000.0 + i * 100) for i in range(5)]
        results = await asyncio.gather(*tasks)
    ...
    if db_pool:
        async with db_pool.acquire() as conn:
            try:
                # Clean up test data
                await conn.execute(
                    "DELETE FROM optimization_runs WHERE depot_id = $1", test_depot_id
                )
                for vid in test_vehicle_ids:
                    await conn.execute("DELETE FROM telemetry WHERE vehicle_id = $1::uuid", vid)
                await conn.execute("DELETE FROM prices WHERE depot_id = $1::uuid", test_depot_id)
            except Exception:
