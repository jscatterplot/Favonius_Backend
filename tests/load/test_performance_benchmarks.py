...
class TestPerformanceBenchmarks:
    ...
    async def message_handler(self, connection_manager, config):
        ...
        return MessageHandler(
            connection_manager=connection_manager, config=config, timescale_client=None
        )
    ...
    async def test_message_processing_performance(self, message_handler):
        ...
        print("BootNotification Performance:")
    ...
    async def test_concurrent_message_processing(self, message_handler):
        ...
        async def process_message(station_id):
            boot_message = {
                "chargingStation": {"model": f"Model_{station_id}", "vendorName": "Test Vendor"},
                "reason": "PowerUp",
            }
        ...
        tasks = [process_message(f"CONCURRENT_STATION_{i}") for i in range(50)]
        ...
        ), f"Average concurrent processing time too high: {avg_processing_time:.4f}s"
    ...
    async def test_connection_manager_performance(self, connection_manager):
        ...
        # Performance assertions
        assert (
            avg_registration_time < 0.01
        ), f"Average registration time too high: {avg_registration_time:.4f}s"
        assert (
            max_registration_time < 0.05
        ), f"Maximum registration time too high: {max_registration_time:.4f}s"
        ...
        print("Connection Manager Performance:")
        print(
            f"  Registration - Average: {avg_registration_time:.4f}s, Max: {max_registration_time:.4f}s"
        )
...
class TestStressTests:
    ...
    async def test_message_processing_under_stress(self, connection_manager, config):
        ...
        for i in range(500):  # 500 concurrent messages
            boot_message = {
                "chargingStation": {"model": f"StressModel_{i}", "vendorName": "Stress Vendor"},
                "reason": "PowerUp",
            }
