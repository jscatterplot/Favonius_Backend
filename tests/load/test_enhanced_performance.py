...
class TestLoadPerformance:
    ...
    async def test_burst_load_scenario(self, test_server):
        ...
        try:
            ...
            boot_results = await asyncio.gather(
                *[sim.boot_notification() for sim in burst_simulators]
            )
            boot_time = time.time() - boot_start

            successful_boots = sum(
                1 for result in boot_results if result[2].get("status") == "Accepted"
            )
            logger.info(
                f"Burst boot time: {boot_time:.2f}s, Success rate: {successful_boots}/{len(burst_simulators)}"
            )
    ...
    async def test_soak_load_scenario(self, test_server):
        ...
        try:
            ...
            # Boot all simulators
            boot_results = await asyncio.gather(
                *[sim.boot_notification() for sim in soak_simulators]
            )
            successful_boots = sum(
                1 for result in boot_results if result[2].get("status") == "Accepted"
            )
    ...
    async def test_chaos_load_scenario(self, test_server):
        ...
        try:
            ...
            # Boot all simulators
            boot_results = await asyncio.gather(
                *[sim.boot_notification() for sim in chaos_simulators]
            )
            successful_boots = sum(
                1 for result in boot_results if result[2].get("status") == "Accepted"
            )
            ...
            while time.time() - start_time < 20:  # 20-second chaos
                ...
                if chaos_operations % 10 == 0:  # Every 10 operations
                    ...
                    await asyncio.gather(
                        *[sim.boot_notification() for sim in simulators_to_disconnect]
                    )
                ...
                if active_simulators:
                    heartbeat_results = await asyncio.gather(
                        *[sim.heartbeat() for sim in active_simulators]
                    )
    ...
    async def test_message_throughput_scenario(self, test_server):
        ...
        try:
            ...
            while time.time() - start_time < 10:
                # Send heartbeats
                heartbeat_results = await asyncio.gather(
                    *[sim.heartbeat() for sim in throughput_simulators]
                )
                message_count += len(heartbeat_results)

                # Send meter values
                meter_results = await asyncio.gather(
                    *[sim.meter_values(1, 22.5) for sim in throughput_simulators]
                )
                message_count += len(meter_results)

                # Send status notifications
                status_results = await asyncio.gather(
                    *[sim.status_notification(1, "Available") for sim in throughput_simulators]
                )
    ...
    async def test_response_time_scenario(self, test_server):
        ...
        for connection_count in [10, 50, 100]:
            ...
            try:
                ...
                for _ in range(5):  # 5 iterations per connection count
                    start_time = time.time()
                    heartbeat_results = await asyncio.gather(
                        *[sim.heartbeat() for sim in simulators]
                    )
        ...
        for connection_count in [10, 50, 100]:
            times = [
                ...
                if rt["connections"] == connection_count
            ]
            avg_time = statistics.mean(times)
            max_time = max(times)

            logger.info(
                f"Response times for {connection_count} connections: avg={avg_time:.3f}s, max={max_time:.3f}s"
            )

            # Response times should be reasonable
            assert (
                avg_time < 1.0
            ), f"Average response time should be under 1s for {connection_count} connections"
            assert (
                max_time < 2.0
            ), f"Max response time should be under 2s for {connection_count} connections"
    ...
    async def test_concurrent_transaction_scenario(self, test_server):
        ...
        try:
            ...
            start_results = await asyncio.gather(
                *[sim.request_start_transaction(1) for sim in transaction_simulators]
            )

            successful_starts = sum(
                1 for result in start_results if result[2].get("status") == "Accepted"
            )
            logger.info(
                f"Concurrent transaction starts: {successful_starts}/{len(transaction_simulators)}"
            )
            ...
            if transaction_ids:
                ...
                event_results = await asyncio.gather(
                    *[
                        ...
                        for sim, txn_id in zip(
                            transaction_simulators[: len(transaction_ids)], transaction_ids
                        )
                    ]
                )

                successful_events = sum(
                    1 for result in event_results if result[2].get("status") == "Accepted"
                )
                logger.info(
                    f"Concurrent transaction events: {successful_events}/{len(transaction_ids)}"
                )

                assert (
                    successful_events >= len(transaction_ids) * 0.9
                ), "Should handle concurrent transaction events"
    ...
    async def test_prometheus_metrics_scenario(self, test_server):
        ...
        try:
            ...
            # Boot all simulators
            boot_results = await asyncio.gather(
                *[sim.boot_notification() for sim in metrics_simulators]
            )
            ...
            for result in boot_results:
                if result[2].get("status") == "Accepted":
                    ...
                    metrics_collector.record_message_sent(
                        "METRICS_TEST", "BootNotificationResponse"
                    )
            ...
            await asyncio.gather(
                *[sim.status_notification(1, "Available") for sim in metrics_simulators]
            )
    ...
    async def test_comprehensive_load_test(self):
        ...
        config = Config(
            websocket=WebSocketConfig(
                port=9002, host="0.0.0.0"  # Use different port to avoid conflict with mock server
            ),
        ...
        burst_simulators = [
            CitrineOSSimulator(f"BURST_TEST_{i:03d}") for i in range(10)
        ...
        soak_simulators = [
            CitrineOSSimulator(f"SOAK_TEST_{i:03d}") for i in range(5)
        ]  # Reduced for faster testing
        await asyncio.gather(*[sim.connect() for sim in soak_simulators])
        ...
        chaos_simulators = [
            CitrineOSSimulator(f"CHAOS_TEST_{i:03d}") for i in range(3)
        ]  # Reduced for faster testing
        await asyncio.gather(*[sim.connect() for sim in chaos_simulators])
        ...
        # Cleanup
        await asyncio.gather(
            *[sim.disconnect() for sim in burst_simulators + soak_simulators + chaos_simulators]
        )
