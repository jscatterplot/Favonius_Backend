...
def sample_charging_profile_data():
    ...
    return {
        ...
        "charging_schedule": {
            ...
            "charging_schedule_period": [{"start_period": 0, "limit": 22.0}],
            "duration": 3600,
        },
    }
...
def sample_display_message_data():
    ...
    return {
        ...
        "message": {"format": "UTF8", "language": "en", "content": "Charging in progress"},
    }
...
def sample_tariff_data():
    ...
    return {
        ...
        "currency": "USD",
        "priority": 1,
        "elements": [{"type": "Energy", "price_per_unit": 0.20, "unit": "kWh", "currency": "USD"}],
    }
...
def sample_privacy_request_data():
    ...
    return {
        ...
        "id_token": {"id_token": "CUSTOMER123", "type": "KeyCode"},
...
def mock_boot_notification():
    ...
    return create_mock_ocpp_message(
        "BootNotification",
        {
            "chargingStation": {
                "model": "TestModel",
...
def mock_boot_notification_response():
    ...
    return create_mock_ocpp_response(
        "BootNotification",
        {
            "status": "Accepted",
...
def mock_heartbeat_response():
    ...
    return create_mock_ocpp_response(
        "Heartbeat", {"currentTime": datetime.now(timezone.utc).isoformat()}
    )
...
def mock_transaction_event_response():
    ...
    return create_mock_ocpp_response("TransactionEvent", {"status": "Accepted"})
...
def test_database_url():
    ...
    return os.environ.get(
        "TEST_DATABASE_URL", "postgresql://favonius_test:test_password@localhost:5433/favonius_test"
    )
...
def assert_solve_time_within_limit(result: "OptimizationResult", limit: float = 30.0):
    ...
    assert result.solve_time < limit, f"Solve time {result.solve_time:.2f}s exceeds {limit}s limit"
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "benchmark: mark test for pytest-benchmark")
    config.addinivalue_line("markers", "database: mark test as requiring real database")
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if "test_" in item.name and not any(
            marker in item.name for marker in ["integration", "e2e", "load"]
        ):
