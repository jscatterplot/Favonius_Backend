...
class Test24HourSimulation:
    ...
    def create_depot_state(
        ...
        return DepotState(
            ...
            battery_soc=0.5,
            prices=(
                prices[current_timestep : current_timestep + n_t]
                if current_timestep + n_t <= len(prices)
                else prices[:n_t]
            ),
    ...
    @pytest.mark.asyncio
    async def test_24_hour_simulation_all_departures_met(self, depot_config, mock_db_pool):
        ...
        if missed:
            ...
            for v in missed[:3]:
                print(
                    f"    {v}: SoC={departures_status[v]['soc_at_departure']:.2f}, "
    ...
    @pytest.mark.asyncio
    async def test_24_hour_with_soc_deviations(self, depot_config, mock_db_pool):
    ...
    @pytest.mark.asyncio
    async def test_24_hour_with_price_spikes(self, depot_config, mock_db_pool):
    ...
    @pytest.mark.asyncio
    @pytest.mark.slow
    async def test_rapid_reoptimization_cycle(self, depot_config, mock_db_pool):
        ...
        for t in range(16):
            state = self.create_depot_state(depot_config, current_socs, prices, departures, t)
            ...
            with patch.object(
                ...
                if result.status == "completed":
                    for v in current_socs:
                        if v in result.schedule:
                            current_socs[v] = (
                                result.schedule[v]["soc"][1]
                                if len(result.schedule[v]["soc"]) > 1
                                else current_socs[v]
                            )
