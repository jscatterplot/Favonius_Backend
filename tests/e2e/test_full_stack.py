...
class TestFullStackE2E:
    ...
    async def test_api_to_optimization_to_dispatch(
        ...
        dispatch_calls = sum(
            1 for cp in mock_ocpp_server.charge_points.values() if cp.set_charging_profile.called
        )
    ...
    async def test_full_pipeline_with_trigger_monitor(
        ...
        with patch.object(StateAssembler, "load_depot_config", new_callable=AsyncMock) as mock_load:
            ...
            # Store expected SoCs
            expected_socs = {
                v: result1.schedule[v]["soc"][48] for v in depot_config.vehicle_capacities
            }
    ...
    async def test_dispatch_verification(
        ...
        with patch.object(StateAssembler, "load_depot_config", new_callable=AsyncMock) as mock_load:
            ...
            await controller.run_optimization("dispatch_test")
    ...
    async def test_partial_dispatch_failure_handling(
        ...
        mock_ocpp_server.charge_points["charger_2"].set_charging_profile = AsyncMock(
            return_value=False
        )
        mock_ocpp_server.charge_points["charger_4"].set_charging_profile = AsyncMock(
