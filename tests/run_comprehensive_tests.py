...
class TestRunner:
    ...
    async def run_all_tests(self, include_performance: bool = True, include_chaos: bool = True) -> Dict[str, Any]:
        """Run all test suites."""
        ...
        print("\n⚡ Phase 3: V2G-Specific Tests")
        print("-" * 30)
        v2g_results = await self._run_test_suite("tests/integration/test_v2g_integration.py", "V2G Integration Tests")
        ...
        if include_performance:
            print("\n⚡ Phase 4: Performance Tests")
            print("-" * 30)
            performance_results = await self._run_test_suite("tests/performance", "Performance Tests")
    ...
    async def _run_test_suite(self, test_path: str, suite_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                ...
                "--tb=short",
                "--durations=10",
                "--junitxml=f"test_results_{suite_name.lower().replace(' ', '_')}.xml"",
                "--html=f"test_report_{suite_name.lower().replace(' ', '_')}.html"",
                "--self-contained-html"
            ]
    ...
    def print_summary(self):
        ...
        for suite_name, results in self.results.items():
            if isinstance(results, dict) and "suite_name" in results:
                ...
                print(f"{status} {results['suite_name']}: {passed}P {failed}F {skipped}S ({duration:.1f}s)")
