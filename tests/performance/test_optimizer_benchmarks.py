...
class TestFleetSizeBenchmarks:
    ...
    @pytest.mark.parametrize("n_vehicles,max_time", [
    ...
    def test_solve_time_by_fleet_size(self, n_vehicles, max_time):
        ...
        assert metrics['solve_time'] < max_time, (
    ...
    def test_scaling_analysis(self):
        ...
        print("-" * 50)
        for r in results:
            print(f"  {r['n_vehicles']:2d} vehicles: {r['solve_time']:.2f}s, "
...
class TestMemoryBenchmarks:
    ...
    @pytest.mark.parametrize("n_vehicles,max_memory_mb", [
        (5, 512),
        (10, 1024),
    ...
    def test_memory_usage_by_fleet_size(self, n_vehicles, max_memory_mb):
        ...
        assert metrics['mem_delta'] < max_memory_mb, (
...
class TestWarmStartBenchmarks:
    ...
    def test_warm_start_speedup(self, n_vehicles):
        ...
        warm_metrics = run_optimization_with_metrics(
            state, config, time_limit=30.0, 
            previous_result=cold_metrics['result']
        )
        ...
        if cold_metrics['solve_time'] < 0.1 and warm_metrics['solve_time'] < 0.1:
            print(f"  Both solves very fast - warm-start benefit not measurable (PASS)")
            return
        
        speedup = cold_metrics['solve_time'] / warm_metrics['solve_time'] if warm_metrics['solve_time'] > 0.001 else 1.0
    ...
    def test_warm_start_consistency(self):
        ...
        for i, obj in enumerate(warm_objectives):
            diff_percent = abs(obj - cold_result.objective_value) / cold_result.objective_value * 100
...
class TestPRDComplianceBenchmarks:
    ...
    def test_prd_warm_start_speedup(self):
        ...
        print(f"  Target: > 3x (or both <0.1s)")
        ...
        if cold_time < 0.1 and warm_time < 0.1:
            print(f"  Both solves very fast (<0.1s) - PASS")
            return
        ...
        assert speedup >= 2.5, (
            f"PRD WARNING: Warm-start speedup {speedup:.2f}x below target"
        )
