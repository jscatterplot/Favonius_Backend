...
class Depot(Base):
    ...
    utility_id = Column(String(100), nullable=True, comment="Utility provider identifier (e.g., 'PG&E', 'SCE')")
...
class Vehicle(Base):
    ...
    vehicle_id = Column(PGUUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    depot_id = Column(PGUUID(as_uuid=True), ForeignKey('depots.depot_id'), nullable=False)
    external_id = Column(String(100), unique=True, nullable=False, comment="Customer's vehicle ID (e.g., 'bus_101')")
    ...
    id_tag = Column(String(100), nullable=True, comment="OCPP idTag used in Authorize messages to map sessions to vehicles")
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class Charger(Base):
    ...
    charger_id = Column(PGUUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    depot_id = Column(PGUUID(as_uuid=True), ForeignKey('depots.depot_id'), nullable=False)
...
class BatteryStorage(Base):
    ...
    battery_id = Column(PGUUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    depot_id = Column(PGUUID(as_uuid=True), ForeignKey('depots.depot_id'), nullable=False)
...
class Schedule(Base):
    ...
    schedule_id = Column(PGUUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    ...
    actual_return_time = Column(DateTime(timezone=True), nullable=True, comment="Updated when vehicle actually returns")
    energy_kwh = Column(Double, nullable=True, comment="Estimated energy consumption (kWh) from surrogate model")
    required_soc = Column(Double, default=1.0)
    dest_depot_id = Column(PGUUID(as_uuid=True), ForeignKey('depots.depot_id'), nullable=True, comment="if different from home depot")
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    __table_args__ = (
        Index('idx_schedules_vehicle_depart', 'vehicle_id', 'departure_time'),
    )
...
class OptimizationRun(Base):
    ...
    trigger_reason = Column(String(50), nullable=False, comment="'scheduled', 'price_spike', 'soc_deviation', 'return_time_deviation', 'interdepot_handoff'")
    ...
    schedule_json = Column(JSONB, nullable=False, comment="Full optimization schedule (includes solver_used metadata)")
...
class ChargingCommand(Base):
    ...
    command_id = Column(PGUUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
...
class InterdepotMessage(Base):
    ...
    message_id = Column(PGUUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('pending', 'acknowledged', 'arrived')", name='interdepot_status_valid'),
...
class TriggerLog(Base):
    ...
    trigger_id = Column(PGUUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    depot_id = Column(PGUUID(as_uuid=True), ForeignKey('depots.depot_id'), nullable=False)
    trigger_type = Column(String(50), nullable=False, comment="'soc_deviation', 'price_change', 'return_time_deviation', 'interdepot_handoff', 'scheduled'")
    trigger_time = Column(DateTime(timezone=True), server_default=func.now())
    details = Column(JSONB, nullable=True, comment="e.g., {vehicle_id, expected_soc, actual_soc, deviation}")
    run_id = Column(PGUUID(as_uuid=True), ForeignKey('optimization_runs.run_id'), nullable=True)
