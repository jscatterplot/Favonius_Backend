    ...
    for proxy_hop in forwarded.split(","):
        for item in proxy_hop.split(";"):
            ...
            if separator and key.lower() == "for":
                parsed = _normalize_forwarded_ip(value)
                if parsed:
                    return parsed

    x_forwarded_for = headers.get("X-Forwarded-For", "") or ""
    for candidate in x_forwarded_for.split(","):
        parsed = _normalize_forwarded_ip(candidate)
        if parsed:
            return parsed

    x_real_ip = headers.get("X-Real-IP", "") or ""
    return _normalize_forwarded_ip(x_real_ip) if x_real_ip else None
...
def _parse_ip_networks(
    ...
    for raw_range in ranges.split(","):
        ...
        try:
            ...
            logger.warning("Invalid GEO_BLOCK_TRUSTED_PROXY_RANGES entry ignored: %s", raw_range)
    return networks
