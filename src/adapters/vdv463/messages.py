...
class VehicleInfo:
    ...
    vehicle_charging_status: str = "Unknown"  # ReadyToCharge | Charging | ChargingImpossible | Unknown
    preconditioning_info: Optional[PreconditioningInfo] = None  # required; use default empty if absent
...
class ChargingProcessInfo:
    ...
    charging_prediction_data: Optional[Dict[str, Any]] = None  # e.g. chargingPredictionDataDepartureTime
...
class ChargingPointInfo:
    ...
    charging_point_status: Optional[str] = None  # Available | Occupied | Reserved | Unavailable | Faulted
    ...
    def __init__(self, schema_dir: Optional[str] = None):
        ...
        if cache_dir.exists():
            ...
            if self.schemas:
                ...
                self.logger.info("VDV 463 schemas loaded from cache", dir=str(cache_dir), count=len(self.schemas))
        ...
        if not self.schemas:
            if _fetch_schemas_to_cache(cache_dir):
                ...
                if self.schemas:
                    ...
                    self.logger.info("VDV 463 schemas fetched from GitHub and cached", count=len(self.schemas))
        ...
        if not self.schemas and bundled_dir.exists():
            ...
            if self.schemas:
                ...
                self.logger.info("VDV 463 schemas loaded from bundled", dir=str(bundled_dir), count=len(self.schemas))

        # 4) Last resort: minimal inline schemas
        if not self.schemas:
            self._create_minimal_schemas()
            self._schema_source = "minimal"
            self.logger.warning("VDV 463 using minimal inline schemas; add bundled schemas or enable network for full validation")
    ...
    def _create_minimal_schemas(self) -> None:
        ...
        self.schemas["MessageStructure"] = {
            ...
            "items": [
                ...
                {"type": "string", "enum": ["BootNotification", "ProvideChargingRequests", "ProvideChargingInformation"]},  # MessageAction
                {"type": "object"},  # Payload
            ],
        }
                ...
                "chargingRequestList": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "chargingInstruction": {"type": "string", "enum": ["Normal", "Changed", "Terminate"]},
                            "chargingRequestData": {
                                "type": "object",
                                "required": ["minTargetSoc", "maxTargetSoc"],
                                "properties": {
                                    "expectedArrivalTimeAtChargingPoint": {"type": "string", "format": "date-time"},
                                    "expectedSocAtArrival": {"type": "number", "minimum": 0, "maximum": 100},
                                    "minTargetSoc": {"type": "number", "minimum": 0, "maximum": 100},
                                    "maxTargetSoc": {"type": "number", "minimum": 0, "maximum": 100},
                                    "requestedTimeForDeparture": {"type": "string", "format": "date-time"},
        ...
        self.logger.info("Created minimal inline schemas (fallback when fetch and bundled unavailable)")
...
def _charging_point_info_to_dict(cp: ChargingPointInfo) -> Dict[str, Any]:
    ...
    if cp.present_power is not None or cp.current_power_kw is not None:
        d["presentPower"] = cp.present_power if cp.present_power is not None else cp.current_power_kw
    if cp.vehicle_info:
        vi = cp.vehicle_info
            ...
            station_list = [
                {
                    ...
                    "chargingPointInfoList": [_charging_point_info_to_dict(cp) for cp in st.charging_point_info_list],
                }
