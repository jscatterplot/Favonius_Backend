...
class PriorityChargingManager:
    ...
    async def use_priority_charging(self, station_id: str, transaction_id: str, 
                                   activate: bool, **kwargs) -> Dict[str, Any]:
        """Handle UsePriorityCharging request."""
        try:
            self.logger.info(f"Priority charging {'activation' if activate else 'deactivation'} for station {station_id}, transaction {transaction_id}")
            
            if activate:
                return await self._activate_priority_charging(station_id, transaction_id, **kwargs)
            else:
                return await self._deactivate_priority_charging(station_id, transaction_id, **kwargs)
    ...
    async def notify_priority_charging(self, station_id: str, transaction_id: str, 
                                     activated: bool, **kwargs) -> Dict[str, Any]:
        """Handle NotifyPriorityCharging request."""
        try:
            self.logger.info(f"Priority charging notification: {'activated' if activated else 'deactivated'} for station {station_id}, transaction {transaction_id}")
            
            # Update priority charging status
            await self._update_priority_charging_status(station_id, transaction_id, activated)
            
            # Store notification
            await self._store_priority_charging_notification(station_id, transaction_id, activated, **kwargs)
    ...
    async def _activate_priority_charging(self, station_id: str, transaction_id: str, 
                                        **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "priorityProfile": {
                    ...
                    "deadline": priority_profile.deadline.isoformat() if priority_profile.deadline else None
                }
            }
    ...
    async def _deactivate_priority_charging(self, station_id: str, transaction_id: str, 
                                          **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Update database
            await self._update_priority_profile_status(priority_profile, PriorityChargingStatus.INACTIVE)
    ...
    async def _create_priority_profile(self, station_id: str, evse_id: int, 
                                     transaction_id: str, **kwargs) -> PriorityChargingProfile:
    ...
    async def _activate_profile(self, profile: PriorityChargingProfile) -> None:
        ...
        self.logger.info(f"Activated priority charging profile {profile.profile_id} for station {profile.station_id}, EVSE {profile.evse_id}")
    ...
    async def _deactivate_profile(self, profile: PriorityChargingProfile) -> None:
        ...
        self.logger.info(f"Deactivated priority charging profile {profile.profile_id} for station {profile.station_id}, EVSE {profile.evse_id}")
    ...
    async def _check_priority_charging_limits(self, station_id: str, evse_id: int) -> bool:
        ...
        # Check EVSE limit
        evse_count = sum(1 for profile in self.active_profiles.get(station_id, {}).values() 
    ...
    async def _get_transaction_data(self, station_id: str, transaction_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def _find_active_priority_profile(self, station_id: str, transaction_id: str) -> Optional[PriorityChargingProfile]:
        """Find active priority profile for a transaction."""
        if station_id not in self.active_profiles:
            return None
        
        for profile in self.active_profiles[station_id].values():
            if profile.transaction_id == transaction_id and profile.status == PriorityChargingStatus.ACTIVE:
                return profile
    ...
    async def _update_priority_profile_status(self, profile: PriorityChargingProfile, 
                                            status: PriorityChargingStatus) -> None:
    ...
    async def _update_priority_charging_status(self, station_id: str, transaction_id: str, 
                                             activated: bool) -> None:
        """Update priority charging status."""
        try:
    ...
    async def _store_priority_charging_notification(self, station_id: str, transaction_id: str, 
                                                   activated: bool, **kwargs) -> None:
        ...
        try:
            ...
            for station_id, profiles in self.active_profiles.items():
                ...
                for evse_id in expired_profiles:
                    ...
                    await self._update_priority_profile_status(profile, PriorityChargingStatus.EXPIRED)
                    
                    self.logger.info(f"Expired priority charging profile {profile.profile_id} for station {station_id}, EVSE {evse_id}")
