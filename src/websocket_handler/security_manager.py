...
class SecurityManager:
    ...
    async def authenticate_station(self, station_id: str, auth_data: Dict[str, Any], 
                                 client_cert: Optional[x509.Certificate] = None) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if await self._is_station_locked_out(station_id):
                await self._log_security_event(
                    station_id, SecurityEventType.FAILED_TO_AUTHENTICATE_AT_CENTRAL_SYSTEM,
            ...
            for method, auth_func in auth_methods:
                if await auth_func(station_id, auth_data, client_cert):
                    ...
                    await self._log_security_event(
                        station_id, SecurityEventType.STARTUP_OF_THE_DEVICE,
            ...
            await self._log_security_event(
                station_id, SecurityEventType.CRITICAL_SECURITY_ERROR,
                f"Authentication error: {str(e)}"
            )
            return False, "Internal authentication error"
    
    async def generate_station_token(self, station_id: str, token_type: str = "Bearer") -> StationAuthToken:
        ...
        try:
            ...
            auth_token = StationAuthToken(
                ...
                token=token,
                token_type=token_type,
                expires_at=datetime.now(timezone.utc) + timedelta(hours=self.config.token_expiry_hours),
    ...
    async def validate_station_token(self, station_id: str, token: str) -> bool:
        ...
        try:
            ...
            if station_id in self.token_cache:
                ...
                if cached_token.token == token and cached_token.expires_at > datetime.now(timezone.utc):
                    # Update usage
    ...
    async def revoke_station_token(self, station_id: str) -> bool:
        ...
        try:
            ...
            await self._log_security_event(
                station_id, SecurityEventType.RECONFIGURATION_SECURITY_PARAMETERS,
    ...
    async def handle_security_event_notification(self, station_id: str, event_type: str,
                                               timestamp: str, tech_info: Optional[str] = None,
    ...
    async def get_security_events(self, station_id: Optional[str] = None,
        ...
        try:
            events = await self.timescale_client.get_security_events(
                station_id, event_type.value if event_type else None,
                start_time, end_time
            )
    ...
    async def validate_client_certificate(self, client_cert: x509.Certificate, 
                                        station_id: str) -> bool:
    ...
    async def _authenticate_client_certificate(self, station_id: str, auth_data: Dict[str, Any],
                                             client_cert: Optional[x509.Certificate]) -> bool:
    ...
    async def _authenticate_bearer_token(self, station_id: str, auth_data: Dict[str, Any],
                                       client_cert: Optional[x509.Certificate]) -> bool:
        """Authenticate using bearer token."""
        token = auth_data.get("bearer_token") or auth_data.get("authorization", "").replace("Bearer ", "")
        if not token:
            return False
        
        return await self.validate_station_token(station_id, token)
    
    async def _authenticate_api_key(self, station_id: str, auth_data: Dict[str, Any],
                                  client_cert: Optional[x509.Certificate]) -> bool:
    ...
    async def _authenticate_basic_auth(self, station_id: str, auth_data: Dict[str, Any],
                                     client_cert: Optional[x509.Certificate]) -> bool:
    ...
    async def _is_station_locked_out(self, station_id: str) -> bool:
        ...
        recent_attempts = [
            ...
            if datetime.now(timezone.utc) - attempt < timedelta(minutes=self.config.lockout_duration_minutes)
        ]
    ...
    async def _record_failed_attempt(self, station_id: str) -> None:
        ...
        self.failed_auth_attempts[station_id] = [
            attempt for attempt in self.failed_auth_attempts[station_id]
    ...
    async def _log_security_event(self, station_id: str, event_type: SecurityEventType,
                                message: str, additional_info: Optional[Dict[str, Any]] = None) -> None:
        """Log security event."""
    ...
    async def _handle_critical_security_event(self, station_id: str, security_event: SecurityEvent) -> None:
        ...
        self.logger.critical(f"Critical security event for station {station_id}: {security_event.event_type.value}")
    ...
    async def _update_token_usage(self, station_id: str, token: str) -> None:
        ...
        await self.timescale_client.update_token_usage(station_id, token, datetime.now(timezone.utc))
    ...
    async def _validate_certificate_chain(self, client_cert: x509.Certificate, station_id: str) -> bool:
