...
from .monitoring import get_logger, health_checker, metrics_collector
...
class HealthCheckServer:
    ...
    async def _readiness_check(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if self._shutting_down:
                return web.json_response(
                    {"status": "not_ready", "reason": "shutting_down"}, status=503
                )
                ...
                return web.json_response(
                    {
                        "status": "not_ready",
                        "checks": {
                            k: v for k, v in results["checks"].items() if k in critical_checks
                        },
                    },
                    status=503,
                )
    ...
    async def _metrics_summary(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            return web.json_response({"metrics": summary, "timestamp": time.time()})
