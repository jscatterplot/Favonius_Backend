...
class ExternalControlManager:
    ...
    async def process_external_limit_signal(self, station_id: str, evse_id: int,
                                         signal_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process external limit signal from EMS/SO/CSO."""
        try:
            self.logger.info(f"Processing external limit signal for station {station_id}, EVSE {evse_id}")
            ...
            if signal.control_type in [ExternalControlType.GRID_CRITICAL, ExternalControlType.EMERGENCY_SHUTDOWN]:
    ...
    async def _parse_external_signal(self, station_id: str, evse_id: int, 
                                   signal_data: Dict[str, Any]) -> Optional[ExternalControlSignal]:
        ...
        try:
            ...
            # Parse timestamps
            response_deadline = datetime.fromisoformat(response_deadline_str.replace('Z', '+00:00')) if response_deadline_str else datetime.now(timezone.utc)
    ...
    async def _check_external_signal_limits(self, station_id: str, evse_id: int) -> bool:
        ...
        # Check EVSE limit
        evse_count = sum(1 for signal in self.active_signals.get(station_id, []) 
                        if signal.evse_id == evse_id)
    ...
    async def _process_signal_by_type(self, signal: ExternalControlSignal) -> Dict[str, Any]:
        ...
        try:
            if signal.control_type == ExternalControlType.GRID_CRITICAL:
                ...
                return {
                    "success": False,
                    "message": f"Unknown control type: {signal.control_type}"
                }
                
        except Exception as e:
            return {
                "success": False,
                "message": f"Error processing signal: {e}"
            }
    ...
    async def _process_grid_critical_signal(self, signal: ExternalControlSignal) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"Grid critical signal received for station {signal.station_id}: {signal.signal_value}")
            ...
            return {
                "success": False,
                "message": f"Error processing grid critical signal: {e}"
            }
    
    async def _process_local_generation_signal(self, signal: ExternalControlSignal) -> Dict[str, Any]:
        """Process local generation signal."""
        try:
            # Local generation signals add capacity
            self.logger.info(f"Local generation signal received for station {signal.station_id}: {signal.signal_value}")
            ...
            return {
                "success": False,
                "message": f"Error processing local generation signal: {e}"
            }
    
    async def _process_demand_response_signal(self, signal: ExternalControlSignal) -> Dict[str, Any]:
        """Process demand response signal."""
        try:
            # Demand response signals adjust charging behavior
            self.logger.info(f"Demand response signal received for station {signal.station_id}: {signal.signal_value}")
            ...
            return {
                "success": False,
                "message": f"Error processing demand response signal: {e}"
            }
    
    async def _process_frequency_regulation_signal(self, signal: ExternalControlSignal) -> Dict[str, Any]:
        """Process frequency regulation signal."""
        try:
            # Frequency regulation signals adjust power based on grid frequency
            self.logger.info(f"Frequency regulation signal received for station {signal.station_id}: {signal.signal_value}")
    ...
    async def _process_voltage_regulation_signal(self, signal: ExternalControlSignal) -> Dict[str, Any]:
        """Process voltage regulation signal."""
        try:
            # Voltage regulation signals adjust reactive power
            self.logger.info(f"Voltage regulation signal received for station {signal.station_id}: {signal.signal_value}")
            ...
            return {
                "success": False,
                "message": f"Error processing voltage regulation signal: {e}"
            }
    
    async def _process_emergency_shutdown_signal(self, signal: ExternalControlSignal) -> Dict[str, Any]:
        """Process emergency shutdown signal."""
        try:
            # Emergency shutdown signals require immediate stop
            self.logger.critical(f"Emergency shutdown signal received for station {signal.station_id}")
            ...
            return {
                "success": False,
                "message": f"Error processing emergency shutdown signal: {e}"
            }
    
    async def _create_external_constraints_profile(self, signal: ExternalControlSignal, **kwargs) -> None:
    ...
    async def _find_active_signal(self, station_id: str, signal_id: str) -> Optional[ExternalControlSignal]:
    ...
    async def _deactivate_signal(self, signal: ExternalControlSignal) -> None:
        ...
        if signal.station_id in self.active_signals:
            self.active_signals[signal.station_id] = [
                s for s in self.active_signals[signal.station_id] 
        ...
        try:
            ...
            for station_id, signals in self.active_signals.items():
                ...
                for signal in expired_signals:
                    ...
                    self.logger.info(f"Expired external signal {signal.signal_id} for station {station_id}")
