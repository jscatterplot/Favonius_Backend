...
class EnhancedOCPPChargePoint(OCPPChargePoint):
    ...
    def on_boot_notification(self, charging_station: ChargingStationType, reason: str, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._initialize_complete_device_model(), "init_device_model"
        )
        ...
        return call_result.BootNotification(
            ...
            status_info=StatusInfoType(
                reason_code="NoError", additional_info="Station registered successfully"
            ),
        )
    ...
    def on_status_notification(
        ...
        **kwargs,
        ...
        self.logger.debug(
            f"Status notification from {self.id}: Connector {connector_id} = {status}"
        )
    ...
    def on_transaction_event(
        self, event_type: TransactionEventEnumType, timestamp: str, transaction_info: Dict, **kwargs
    ):
    ...
    def on_meter_values(self, evse_id: int, meter_value: list, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._process_meter_values(evse_id, meter_value), f"process_meter_values:{evse_id}"
        )
    ...
    def on_heartbeat(self, **kwargs):
        ...
        self._task_supervisor.create_task(
            self.connection_manager.update_heartbeat(self.id), "update_heartbeat"
        )
    ...
    def on_data_transfer(
        self, vendor_id: str, message_id: str, data: Optional[str] = None, **kwargs
    ):
        ...
        return call_result.DataTransfer(status="Accepted", data=None)

    @on(Action.notify_ev_charging_needs)
    def on_notify_ev_charging_needs(self, evse_id: int, charging_needs: Dict, **kwargs):
    ...
    def on_notify_ev_charging_schedule(
        self, evse_id: int, time_base: str, charging_schedule: Dict, **kwargs
    ):
    ...
    async def send_charging_profile(self, evse_id: int, charging_profile: Dict) -> bool:
        ...
        try:
            ...
            request = call.SetChargingProfile(evse_id=evse_id, charging_profile=charging_profile)
    ...
    async def send_der_control(self, der_control: Dict) -> bool:
        ...
        try:
            ...
            request = call.SetDERControl(der_control=der_control)
    ...
    async def _store_connector_status(
        self, connector_id: int, status: str, error_code: str, timestamp: str
    ):
    ...
    async def _store_transaction_event_v2g(
        ...
        try:
            ...
            transaction_data = {
                ...
                "iso15118_certificate_hash_data": json.dumps(
                    kwargs.get("iso15118CertificateHashData", [])
                ),
            }
            ...
            if is_v2g_event:
                self.logger.info(
                    f"V2G transaction event: {trigger_reason} for transaction {transaction_id}"
                )
                if operation_mode:
    ...
    async def _process_v2g_meter_values(self, meter_values: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            for meter_value in meter_values:
                meter_value.get("timestamp")
                sampled_values = meter_value.get("sampledValue", [])

                for sampled_value in sampled_values:
                    measurand = sampled_value.get("measurand", "")
                    value = sampled_value.get("value")
                    sampled_value.get("unitOfMeasure", {}).get("unit", "")
            ...
            for meter_value in meter_values:
                ...
                for sampled_value in sampled_values:
                    ...
                    if measurand == "Power.Active.Import":
                        telemetry_data["power_kw"] = (
                            float(value) / 1000 if unit == "W" else float(value)
                        )
                    elif measurand == "Power.Active.Export":
                        telemetry_data["power_discharge_kw"] = (
                            float(value) / 1000 if unit == "W" else float(value)
                        )
                    elif measurand == "Power.Active.Setpoint":
                        telemetry_data["power_setpoint_kw"] = (
                            float(value) / 1000 if unit == "W" else float(value)
                        )
                    elif measurand == "Power.Active.Residual":
                        telemetry_data["power_residual_kw"] = (
                            float(value) / 1000 if unit == "W" else float(value)
                        )
                    elif measurand == "Energy.Active.Import.Register":
                        telemetry_data["energy_kwh"] = (
                            float(value) / 1000 if unit == "Wh" else float(value)
                        )
                    elif measurand == "Energy.Active.Export.Register":
                        telemetry_data["energy_discharged_kwh"] = (
                            float(value) / 1000 if unit == "Wh" else float(value)
                        )
                    elif measurand == "SoC":
                        ...
                        telemetry_data["reactive_power_import_kvar"] = (
                            float(value) / 1000 if unit == "var" else float(value)
                        )
                    elif measurand == "Power.Reactive.Export":
                        telemetry_data["reactive_power_export_kvar"] = (
                            float(value) / 1000 if unit == "var" else float(value)
                        )
    ...
    async def _store_ev_charging_needs(self, evse_id: int, charging_needs: Dict):
        ...
        try:
            ...
            if v2x_charging_parameters:
                v2x_params = {
                    "ev_maximum_discharge_power": v2x_charging_parameters.get(
                        "evMaximumDischargePower"
                    ),
                    "ev_minimum_discharge_power": v2x_charging_parameters.get(
                    ...
                    "ev_target_energy_request": v2x_charging_parameters.get(
                        "evTargetEnergyRequest"
                    ),
                    "ev_maximum_energy_request": v2x_charging_parameters.get(
                        "evMaximumEnergyRequest"
                    ),
                    "ev_minimum_energy_request": v2x_charging_parameters.get(
                        "evMinimumEnergyRequest"
                    ),
                    "ev_present_active_power": v2x_charging_parameters.get("evPresentActivePower"),
                    "ev_present_reactive_power": v2x_charging_parameters.get(
                        "evPresentReactivePower"
                    ),
                    "soc": v2x_charging_parameters.get("soc"),
                    "capacity": v2x_charging_parameters.get("capacity"),
                    "control_mode": v2x_charging_parameters.get(
                        "controlMode"
            ...
            if der_charging_parameters:
                der_params = {
                    ...
                    "supported_controls": der_charging_parameters.get(
                        "supportedControls"
                    ),  # Array of DERControlEnumType
                }
            ...
            if v2x_params or der_params:
                self.logger.info(
                    f"V2G charging needs detected for {self.id}, EVSE {evse_id}: "
    ...
    async def _store_ev_charging_schedule(
        self, evse_id: int, time_base: str, charging_schedule: Dict
    ):
        """Store EV charging schedule."""
        try:
            schedule_data = {
    ...
    def on_get_variables(self, get_variable_data: list, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_get_variables(get_variable_data), "get_variables"
        )
    ...
    def on_set_variables(self, set_variable_data: list, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_set_variables(set_variable_data), "set_variables"
        )
    ...
    def on_get_base_report(self, request_id: int, report_base: str, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_get_base_report(request_id, report_base), "get_base_report"
        )

        return call_result.GetBaseReport()

    @on(Action.notify_report)
    def on_notify_report(
        self,
        request_id: int,
        generated_at: str,
        tbc: bool,
    ...
    def on_get_charging_profiles(
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_get_charging_profiles(
                request_id, evse_id, charging_profile_id, charging_profile_purpose, stack_level
            ),
    ...
    def on_clear_charging_profile(
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_clear_charging_profile(
    ...
    def on_set_charging_profile(self, evse_id: int, charging_profile: Dict[str, Any], **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_set_charging_profile(evse_id, charging_profile), "set_charging_profile"
        )
    ...
    async def on_request_start_transaction(
        ...
        evse_id_token: Optional[dict] = None,
        **kwargs,
    ):
    ...
    def on_request_stop_transaction(
        self, transaction_id: str, reason: Optional[str] = None, **kwargs
    ):
        ...
        self._task_supervisor.create_task(
            self._handle_request_stop_transaction(transaction_id, reason),
    ...
    def on_change_availability(
        self, operational_status: str, evse: Optional[dict] = None, **kwargs
    ):
        ...
        self._task_supervisor.create_task(
            self._handle_change_availability(operational_status, evse), "change_availability"
        )
    ...
    def on_trigger_message(self, requested_message: str, evse: Optional[dict] = None, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_trigger_message(requested_message, evse), "trigger_message"
        )
    ...
    def on_unlock_connector(self, evse_id: int, connector_id: int, **kwargs):
        ...
        self.logger.info(
            f"UnlockConnector from {self.id}: EVSE {evse_id}, Connector {connector_id}"
        )

        # Process unlock connector request
        self._task_supervisor.create_task(
            self._handle_unlock_connector(evse_id, connector_id), "unlock_connector"
        )
    ...
    def on_get_15118_ev_certificate(
        self, certificate_type: str, exi_request: Optional[str] = None, **kwargs
    ):
        ...
        self._task_supervisor.create_task(
            self._handle_get_15118_ev_certificate(certificate_type, exi_request),
    ...
    def on_certificate_signed(
        self, certificate_type: str, certificate_chain: List[str], exi_response: str, **kwargs
    ):
        ...
        self._task_supervisor.create_task(
            self._handle_certificate_signed(certificate_type, certificate_chain, exi_response),
            "certificate_signed",
        )
    ...
    def on_install_certificate(self, certificate_type: str, certificate: str, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_install_certificate(certificate_type, certificate), "install_certificate"
        )
    ...
    def on_delete_certificate(self, certificate_hash_data: List[Dict[str, Any]], **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_delete_certificate(certificate_hash_data), "delete_certificate"
        )
    ...
    def on_get_installed_certificate_ids(self, certificate_type: Optional[str] = None, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_get_installed_certificate_ids(certificate_type),
    ...
    def on_sign_certificate(
        self, certificate_type: str, certificate_signing_request: str, **kwargs
    ):
        ...
        self._task_supervisor.create_task(
            self._handle_sign_certificate(certificate_type, certificate_signing_request),
            "sign_certificate",
        )
    ...
    def on_security_event_notification(
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_security_event_notification(
    ...
    def on_get_log(
        ...
        **kwargs,
        ...
        # Process log request
        self._task_supervisor.create_task(
            self._handle_get_log(log_type, request_id, retry_count, retry_interval), "get_log"
        )
    ...
    def on_log_status_notification(self, status: str, request_id: int, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_log_status_notification(status, request_id), "log_status_notification"
        )
    ...
    def on_notify_event(
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_notify_event(event_type, timestamp, tech_info, additional_info),
            "notify_event",
        )
    ...
    def on_unpublish_firmware(self, checksum: str, **kwargs):
        ...
        self._task_supervisor.create_task(
            self._handle_unpublish_firmware(checksum), "unpublish_firmware"
        )
    ...
    def on_firmware_status_notification(
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_firmware_status_notification(status, request_id, location),
    ...
    def on_get_monitoring_report(
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_get_monitoring_report(
    ...
    def on_set_variable_monitoring(
        self,
        component_name: str,
        variable_name: str,
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_set_variable_monitoring(
    ...
    def on_clear_variable_monitoring(
        self, component_name: str, variable_name: str, monitoring_criterion: str, **kwargs
    ):
        """Handle ClearVariableMonitoring request."""
        self.logger.info(
            f"ClearVariableMonitoring from {self.id}: {component_name}.{variable_name}"
        )

        # Process clear variable monitoring request
        self._task_supervisor.create_task(
            self._handle_clear_variable_monitoring(
    ...
    def on_notify_monitoring_report(
        self,
        request_id: int,
        monitoring_base: str,
        monitoring_criterion: Optional[str] = None,
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_notify_monitoring_report(
                request_id, monitoring_base, monitoring_criterion, component_name, variable_name
            ),
    ...
    def on_customer_information(
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_customer_information(
    ...
    if _act_uds is not None:
        ...
        @on(_act_uds)
        def on_update_dynamic_schedule(
            self,
            ...
            **kwargs,
            ...
            self._task_supervisor.create_task(
                self._handle_update_dynamic_schedule(
                    charging_profile_id, limit, discharging_limit, setpoint, setpoint_reactive
                ),
    ...
    if _act_pds is not None:
        ...
        def on_pull_dynamic_schedule_update(self, charging_profile_id: int, **kwargs):
            self._task_supervisor.create_task(
                self._handle_pull_dynamic_schedule_update(charging_profile_id),
    ...
    def on_notify_charging_limit(
        ...
        **kwargs,
        ...
        self._task_supervisor.create_task(
            self._handle_notify_charging_limit(charging_schedule, evse_id, charging_limit),
    ...
    def on_cleared_charging_limit(
        self, charging_limit_source: str, evse_id: Optional[int] = None, **kwargs
    ):
        """Handle ClearedChargingLimit request."""
        self._task_supervisor.create_task(
            self._handle_cleared_charging_limit(charging_limit_source, evse_id),
    ...
    if _act_upc is not None:
        ...
        def on_use_priority_charging(self, transaction_id: str, activate: bool, **kwargs):
            self._task_supervisor.create_task(
                self._handle_use_priority_charging(transaction_id, activate),
    ...
    if _act_npc is not None:
        ...
        def on_notify_priority_charging(self, transaction_id: str, activated: bool, **kwargs):
            self._task_supervisor.create_task(
                self._handle_notify_priority_charging(transaction_id, activated),
    ...
    if _act_naet is not None:
        ...
        def on_notify_allowed_energy_transfer(self, allowed_energy_transfer: List[str], **kwargs):
            self._task_supervisor.create_task(
                self._handle_notify_allowed_energy_transfer(allowed_energy_transfer),
    ...
    if _act_sdc is not None:
        ...
        def on_set_der_control(self, der_control: Dict, **kwargs):
            self._task_supervisor.create_task(
                self._handle_set_der_control(der_control), "set_der_control"
            )
    ...
    if _act_gdc is not None:
        ...
        def on_get_der_control(self, control_id: Optional[int] = None, **kwargs):
            self._task_supervisor.create_task(
                self._handle_get_der_control(control_id), "get_der_control"
            )
    ...
    if _act_rdc is not None:
        ...
        def on_report_der_control(self, der_control: List[Dict], **kwargs):
            self._task_supervisor.create_task(
                self._handle_report_der_control(der_control), "report_der_control"
            )
    ...
    if _act_cdc is not None:
        ...
        def on_clear_der_control(self, control_id: Optional[int] = None, **kwargs):
            self._task_supervisor.create_task(
                self._handle_clear_der_control(control_id), "clear_der_control"
            )
    ...
    if _act_nda is not None:
        ...
        def on_notify_der_alarm(
            ...
            **kwargs,
            ...
            self._task_supervisor.create_task(
                self._handle_notify_der_alarm(
                    control_type, alarm_ended, grid_event_fault, timestamp
                ),
                "notify_der_alarm",
            )
    ...
    if _act_ndss is not None:
        ...
        def on_notify_der_start_stop(
            ...
            **kwargs,
            ...
            self._task_supervisor.create_task(
                self._handle_notify_der_start_stop(control_id, started, superseded_id, timestamp),
        ...
        def on_notify_der_start_stop(
            self, control_id: int, started: bool, superseded_id=None, timestamp=None, **kwargs
        ):
            return {"status": "Accepted"}
    ...
    if _act_afrr is not None:
        ...
        def on_afrr_signal(self, signal: float, timestamp: str, **kwargs):
            self._task_supervisor.create_task(
                self._handle_afrr_signal(signal, timestamp), "afrr_signal"
            )
    ...
    async def _handle_get_variables(self, get_variable_data: list):
        ...
        try:
            ...
            request = call.GetVariablesResponse(get_variable_result=results)
    ...
    async def _handle_set_variables(self, set_variable_data: list):
        ...
        try:
            ...
            request = call.SetVariablesResponse(set_variable_result=results)
            await self.call(request)
    ...
    async def _handle_get_base_report(self, request_id: int, report_base: str):
        ...
        try:
            await self.device_model.get_base_report(self.id, report_base)

            # Send GetBaseReportResponse
            from ocpp.v201 import call

            request = call.GetBaseReportResponse(
                status="Accepted", status_info={"reason_code": "NoError"}
            )
            await self.call(request)
    ...
    async def _handle_notify_report(
        self, request_id: int, generated_at: str, tbc: bool, seq_no: int, report_data: list
    ):
    ...
    async def _handle_get_charging_profiles(
        self,
        request_id: int,
        evse_id: int,
        ...
        try:
            result = await self.charging_profile_manager.get_charging_profiles(
                self.id, evse_id, charging_profile_id, charging_profile_purpose, stack_level
            )
            ...
            request = call.GetChargingProfilesResponse(
                status=result["status"], status_info=result.get("statusInfo")
            )
            await self.call(request)
    ...
    async def _handle_clear_charging_profile(
        self,
        charging_profile_id: Optional[int],
        charging_profile_purpose: Optional[str],
        stack_level: Optional[int],
    ...
    async def _handle_get_composite_schedule(
        self, request_id: int, evse_id: int, duration: int, charging_rate_unit: Optional[str]
    ):
    ...
    async def _handle_report_charging_profiles(
        self, request_id: int, evse_id: int, charging_profile: list
    ):
    ...
    async def _handle_reset(self, reset_type: str, evse_id: Optional[int]):
        ...
        try:
            ...
            request = call.ResetResponse(status="Accepted", status_info={"reason_code": "NoError"})
            await self.call(request)
    ...
    async def _handle_get_15118_ev_certificate(
        self, certificate_type: str, exi_request: Optional[str]
    ):
        """Handle Get15118EVCertificate request."""
        try:
            cert_type = CertificateType(certificate_type)
            result = await self.certificate_manager.get_15118_ev_certificate(
                self.id, cert_type, exi_request
            )
    ...
    async def _handle_certificate_signed(
        self, certificate_type: str, certificate_chain: List[str], exi_response: str
    ):
        """Handle CertificateSigned notification."""
        try:
            cert_type = CertificateType(certificate_type)
            result = await self.certificate_manager.certificate_signed(
                self.id, cert_type, certificate_chain, exi_response
            )
    ...
    async def _handle_install_certificate(self, certificate_type: str, certificate: str):
        ...
        try:
            cert_type = CertificateType(certificate_type)
            result = await self.certificate_manager.install_certificate(
                self.id, cert_type, certificate
            )
    ...
    async def _handle_delete_certificate(self, certificate_hash_data: List[Dict[str, Any]]):
        ...
        try:
            result = await self.certificate_manager.delete_certificate(
                self.id, certificate_hash_data
            )
    ...
    async def _handle_get_installed_certificate_ids(self, certificate_type: Optional[str]):
        ...
        try:
            ...
            result = await self.certificate_manager.get_installed_certificate_ids(
                self.id, cert_type
            )
    ...
    async def _handle_sign_certificate(
        self, certificate_type: str, certificate_signing_request: str
    ):
        """Handle SignCertificate request."""
        try:
            cert_type = CertificateType(certificate_type)
            result = await self.certificate_manager.sign_certificate(
                self.id, cert_type, certificate_signing_request
            )
    ...
    async def _handle_security_event_notification(
        self,
        event_type: str,
        timestamp: str,
    ...
    async def _handle_notify_event(
        self,
        event_type: str,
        timestamp: str,
        tech_info: Optional[str],
    ...
    async def _handle_publish_firmware(
        self,
        location: str,
        retrieve_date_time: str,
        request_id: int,
        ...
        request_stop_time: Optional[str],
    ):
        """Handle PublishFirmware request."""
        try:
    ...
    async def _handle_update_firmware(
        self,
        location: str,
        retrieve_date_time: str,
        request_id: int,
    ...
    async def _handle_firmware_status_notification(
        self, status: str, request_id: Optional[int], location: Optional[str]
    ):
        """Handle FirmwareStatusNotification."""
        try:
    ...
    async def _handle_get_monitoring_report(
        self,
        request_id: int,
        monitoring_base: str,
    ...
    async def _handle_set_variable_monitoring(
        self,
        component_name: str,
        variable_name: str,
        monitoring_criterion: str,
        ...
        try:
            ...
            request = call.SetVariableMonitoringResponse(
                status=result["status"], statusInfo=result.get("statusInfo")
            )
            await self.call(request)
    ...
    async def _handle_clear_variable_monitoring(
        self, component_name: str, variable_name: str, monitoring_criterion: str
    ):
        """Handle ClearVariableMonitoring request."""
        try:
    ...
    async def _handle_notify_monitoring_report(
        self,
        request_id: int,
        monitoring_base: str,
        monitoring_criterion: Optional[str],
    ...
    async def _handle_set_display_message(
        self, message_info, evse_id: Optional[int], connector_id: Optional[int]
    ):
        """Handle SetDisplayMessage request."""
        try:
    ...
    async def _handle_clear_display_message(
        self, message_id: Optional[str], evse_id: Optional[int], connector_id: Optional[int]
    ):
        """Handle ClearDisplayMessage request."""
        try:
    ...
    async def _handle_customer_information(
        self,
        request_id: int,
    ...
    async def _handle_delete_customer_information(
        self,
        request_id: int,
        customer_certificate_id: Optional[str],
        id_token: Optional[IdTokenType],
    ...
    async def _handle_update_dynamic_schedule(
        self,
        charging_profile_id: int,
        limit: Optional[float] = None,
        discharging_limit: Optional[float] = None,
        ...
        try:
            self.logger.info(
                f"UpdateDynamicSchedule from {self.id}: profile_id={charging_profile_id}, "
            ...
            if self.charging_profile_manager:
                ...
                self.logger.info(
                    f"Updated dynamic schedule for profile {charging_profile_id}: {result}"
                )
            else:
    ...
    async def _handle_pull_dynamic_schedule_update(self, charging_profile_id: int):
        ...
        try:
            self.logger.info(
                f"PullDynamicScheduleUpdate from {self.id}: profile_id={charging_profile_id}"
            )
    ...
    async def _handle_notify_charging_limit(
        self,
        charging_schedule: Optional[List] = None,
        evse_id: Optional[int] = None,
        charging_limit: Dict = None,
        ...
        try:
            self.logger.info(
                f"NotifyChargingLimit from {self.id}: evse_id={evse_id}, "
    ...
    async def _handle_cleared_charging_limit(
        self, charging_limit_source: str, evse_id: Optional[int] = None
    ):
        """Handle ClearedChargingLimit request."""
        try:
            self.logger.info(
                f"ClearedChargingLimit from {self.id}: source={charging_limit_source}, "
                f"evse_id={evse_id}"
            )
    ...
    async def _handle_use_priority_charging(self, transaction_id: str, activate: bool):
        ...
        try:
            self.logger.info(
                f"UsePriorityCharging from {self.id}: transaction_id={transaction_id}, "
                f"activate={activate}"
            )
            ...
            if self.priority_charging_manager:
                ...
                self.logger.info(
                    f"Priority charging {'activated' if activate else 'deactivated'} for transaction {transaction_id}: {result}"
                )
            else:
    ...
    async def _handle_notify_priority_charging(self, transaction_id: str, activated: bool):
        ...
        try:
            self.logger.info(
                f"NotifyPriorityCharging from {self.id}: transaction_id={transaction_id}, "
