...
class TelemetryIngestionService:
    ...
    async def _handle_optimization_decision(self, data: Dict[str, Any]) -> None:
        ...
        try:
            decision_data = {
                "time": datetime.fromisoformat(data["timestamp"].replace("Z", "+00:00")),
                "optimization_window_start": datetime.fromisoformat(
                    data["window_start"].replace("Z", "+00:00")
                ),
                "optimization_window_end": datetime.fromisoformat(
                    data["window_end"].replace("Z", "+00:00")
                ),
                "fleet_operator_id": data.get("fleet_operator_id"),
                "site_id": data.get("site_id"),
    ...
    async def _handle_grid_signal(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Received grid signal: {data['signal_type']} = {data['signal_value']}"
            )
    ...
    async def _process_station_batch(self, station_id: str) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"Processed batch for station {station_id}: {len(batch_data)} records"
            )
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "buffer_sizes": {
                station_id: len(buffer) for station_id, buffer in self.batch_buffer.items()
            },
            "last_batch_times": {
                station_id: time.isoformat() for station_id, time in self.last_batch_time.items()
            },
        }
