...
class ConfigValidator:
    ...
    async def _validate_websocket(self) -> bool:
        ...
        try:
            ...
            if self.config.websocket.max_connections <= 0:
                self.logger.error(f"Invalid max connections: {self.config.websocket.max_connections}")
                return False
            
            if self.config.websocket.heartbeat_interval <= 0:
                self.logger.error(f"Invalid heartbeat interval: {self.config.websocket.heartbeat_interval}")
                return False
    ...
    async def _validate_monitoring(self) -> bool:
        ...
        try:
            if self.config.monitoring.metrics_port <= 0 or self.config.monitoring.metrics_port > 65535:
                self.logger.error(f"Invalid metrics port: {self.config.monitoring.metrics_port}")
                return False
            
            if self.config.monitoring.health_check_port <= 0 or self.config.monitoring.health_check_port > 65535:
                self.logger.error(f"Invalid health check port: {self.config.monitoring.health_check_port}")
                return False
