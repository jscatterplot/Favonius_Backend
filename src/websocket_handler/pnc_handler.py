...
class PnCHandler:
    ...
    async def authorize_plug_and_charge(
        self,
        station_id: str,
        evse_id: int,
        id_token: str,
        certificate_chain: Optional[List[str]] = None,
        contract_certificate: Optional[str] = None,
    ) -> AuthorizationResult:
        ...
        try:
            ...
            if auth_method == AuthorizationMethod.PNC:
                return await self._authorize_pnc(
                    station_id, evse_id, certificate_chain, contract_certificate
                )
            else:
            ...
            return AuthorizationResult(status=PnCStatus.REJECTED, additional_info={"error": str(e)})

    async def validate_contract_certificate(
        self, contract_certificate: str
    ) -> Tuple[bool, Optional[ContractInfo]]:
    ...
    async def revoke_contract(self, contract_id: str, reason: str) -> bool:
        ...
        try:
            ...
            await self.timescale_client.revoke_contract(
                contract_id, reason, datetime.now(timezone.utc)
            )
    ...
    async def _determine_authorization_method(
        self, id_token: str, certificate_chain: Optional[List[str]]
    ) -> AuthorizationMethod:
    ...
    async def _authorize_pnc(
        self,
        station_id: str,
        evse_id: int,
        certificate_chain: Optional[List[str]],
        contract_certificate: Optional[str],
    ) -> AuthorizationResult:
        ...
        try:
            ...
            if certificate_chain and self.config.require_certificate_chain_validation:
                if not await self.validate_certificate_chain(certificate_chain):
                    return AuthorizationResult(
                        status=PnCStatus.REJECTED, certificate_status="InvalidCertificateChain"
                    )

            # Validate contract certificate
            if contract_certificate:
                is_valid, contract_info = await self.validate_contract_certificate(
                    contract_certificate
                )
                if not is_valid or not contract_info:
                    return AuthorizationResult(
                        status=PnCStatus.REJECTED, certificate_status="InvalidContractCertificate"
                    )

                # Check contract status
                if contract_info.status != ContractStatus.VALID:
                    return AuthorizationResult(
                        status=PnCStatus.REJECTED, contract_status=contract_info.status.value
                    )

                # Check contract validity period
                now = datetime.now(timezone.utc)
                if now < contract_info.valid_from or now > contract_info.valid_to:
                    return AuthorizationResult(status=PnCStatus.EXPIRED, contract_status="Expired")

                # Validate energy contract if enabled
                if (
                    self.config.enable_energy_contract_validation
                    and contract_info.energy_contract_id
                ):
                ...
                if self.config.enable_tariff_validation and contract_info.tariff_id:
                    if not await self._validate_tariff(contract_info.tariff_id):
                        return AuthorizationResult(
                            status=PnCStatus.REJECTED, tariff_id=contract_info.tariff_id
                        )
            ...
            return AuthorizationResult(
                status=PnCStatus.REJECTED, certificate_status="NoContractCertificate"
            )
    ...
    async def _authorize_eim(
        self, station_id: str, evse_id: int, id_token: str
    ) -> AuthorizationResult:
        ...
        try:
            ...
            if not token_info:
                return AuthorizationResult(
                    status=PnCStatus.REJECTED, additional_info={"reason": "Unknown ID token"}
                )
            ...
            if token_info.get("expires_at") and token_info["expires_at"] < datetime.now(
                ...
                return AuthorizationResult(
                    status=PnCStatus.EXPIRED, additional_info={"reason": "ID token expired"}
                )

            # Check if token is blocked
            if token_info.get("blocked", False):
                return AuthorizationResult(
                    status=PnCStatus.BLOCKED, additional_info={"reason": "ID token blocked"}
                )
            ...
            if token_info.get("require_credit", False):
                ...
                if balance <= 0:
                    return AuthorizationResult(
                        status=PnCStatus.NO_CREDIT, additional_info={"balance": balance}
                    )
    ...
    async def _extract_contract_info(self, certificate: x509.Certificate) -> ContractInfo:
        ...
        try:
            ...
            contract_info = ContractInfo(
                ...
                certificate_chain=[],
                contract_certificate=base64.b64encode(
    ...
    async def _is_trusted_root_certificate(self, root_cert: x509.Certificate) -> bool:
        ...
        try:
            ...
            for trusted_root in trusted_roots:
                if root_cert.fingerprint(hashes.SHA256()) == trusted_root.fingerprint(
                    hashes.SHA256()
                ):
                    return True
    ...
    async def _check_evse_compatibility(
        self, station_id: str, evse_id: int, contract_info: ContractInfo
    ) -> bool:
        """Check if EVSE is compatible with contract."""
        try:
            # Get EVSE capabilities
            evse_capabilities = await self.timescale_client.get_evse_capabilities(
                station_id, evse_id
            )
