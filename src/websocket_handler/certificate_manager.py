...
class CertificateManager:
    ...
    def __init__(
        ...
        self.cache_manager = cache_manager or CacheManager(
            max_size=500, default_ttl=timedelta(seconds=600)
        )
    ...
    async def get_15118_ev_certificate(
        self, station_id: str, certificate_type: CertificateType, exi_request: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def install_certificate(
        self, station_id: str, certificate_type: CertificateType, certificate: str
    ) -> Dict[str, Any]:
        """Install certificate."""
        try:
    ...
    async def delete_certificate(
        self, station_id: str, certificate_hash_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def get_installed_certificate_ids(
        self, station_id: str, certificate_type: Optional[CertificateType] = None
    ) -> Dict[str, Any]:
    ...
    async def sign_certificate(
        self, station_id: str, certificate_type: CertificateType, certificate_signing_request: str
    ) -> Dict[str, Any]:
    ...
    async def _get_certificate(
        self, station_id: str, certificate_type: CertificateType
        ...
        # Get from database
        certificate_data = await self.timescale_client.get_certificate(
            station_id, certificate_type.value
        )
    ...
    async def _check_certificate_limits(
        self, station_id: str, certificate_type: CertificateType
    ) -> bool:
        """Check certificate store limits."""
        try:
            current_count = await self.timescale_client.count_certificates(
                station_id, certificate_type.value
            )
    ...
    async def _generate_certificate_hash(self, certificate_info: CertificateInfo) -> Dict[str, Any]:
        ...
        try:
            ...
            issuer_key_hash = hashlib.sha256(
                certificate.issuer.public_key().public_bytes(
    ...
    async def _get_installed_certificates(
        self, station_id: str, certificate_type: Optional[CertificateType]
    ) -> List[CertificateInfo]:
        """Get installed certificates."""
        certificates_data = await self.timescale_client.get_installed_certificates(
            station_id, certificate_type.value if certificate_type else None
        )
    ...
    async def _find_certificate_by_hash(
        self,
        station_id: str,
        certificate_type: CertificateType,
        hash_algorithm: str,
    ...
    async def _validate_csr(
        self, csr: x509.CertificateSigningRequest, certificate_type: CertificateType
    ) -> Dict[str, Any]:
    ...
    async def _sign_certificate_request(
        self, csr: x509.CertificateSigningRequest, certificate_type: CertificateType
    ) -> str:
    ...
    async def validate_contract_certificate(
        self, station_id: str, contract_certificate: str, emaid: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if certificate_info.certificate_chain:
                chain_validation = await self._validate_certificate_chain(
            ...
            return {
                ...
                "certificate_info": {
                    ...
                    "serial_number": certificate_info.serial_number,
                    "valid_from": (
                        certificate_info.valid_from.isoformat()
                        if certificate_info.valid_from
                        else None
                    ),
                    "valid_to": (
                        certificate_info.valid_to.isoformat() if certificate_info.valid_to else None
                    ),
                },
            }
    ...
    async def get_v2g_certificate_chain(
        self, station_id: str, certificate_type: CertificateType
    ) -> Dict[str, Any]:
    ...
    async def handle_pnc_authorization(
        self,
        station_id: str,
        contract_certificate: str,
        emaid: str,
        charging_needs: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            cert_validation = await self.validate_contract_certificate(
                station_id, contract_certificate, emaid
            )
            if not cert_validation["valid"]:
                return {
                    "status": "Rejected",
            ...
            await self._store_authorization_record(
                station_id, emaid, contract_certificate, charging_needs
            )
    ...
    async def install_v2g_certificate(
        self,
        station_id: str,
        certificate_type: CertificateType,
        certificate: str,
        ...
        try:
            ...
            # Install certificate
            installation_result = await self._install_certificate(
                station_id, certificate_type, certificate_info, certificate_chain or [certificate]
            )

            if installation_result["status"] == "Accepted":
                self.logger.info(
                    f"Installed V2G {certificate_type.value} certificate for {station_id}"
                )
    ...
    async def _validate_v2g_certificate_requirements(
        self, certificate_info: CertificateInfo
    ) -> Dict[str, Any]:
    ...
    async def _validate_contract_certificate_requirements(
    ...
    async def _check_authorization_status(self, station_id: str, emaid: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if emaid and "@emaid" in emaid:
                return {"authorized": True, "reason": "Valid eMAID"}
            else:
                return {"authorized": False, "reason": "Invalid eMAID format"}

        except Exception as e:
            self.logger.error(f"Error checking authorization status: {e}")
            return {"authorized": False, "reason": f"Authorization check failed: {e}"}

    async def _store_authorization_record(
        self,
        station_id: str,
        emaid: str,
        contract_certificate: str,
        charging_needs: Optional[Dict[str, Any]],
    ) -> None:
        """Store authorization record."""
        try:
