...
class DiagnosticsManager:
    ...
    async def get_log(self, station_id: str, log_type: LogType, request_id: int,
                     retry_count: Optional[int] = None, retry_interval: Optional[int] = None) -> Dict[str, Any]:
    ...
    async def handle_log_status_notification(self, station_id: str, request_id: int,
    ...
    async def handle_notify_event(self, station_id: str, event_type: str, timestamp: str,
    ...
    async def _collect_logs(self, station_id: str, log_type: LogType, request_id: int) -> None:
        ...
        try:
            # Create log file
            log_filename = f"{station_id}_{log_type.value}_{request_id}_{int(datetime.now().timestamp())}.log"
    ...
    async def _collect_security_logs(self, station_id: str, log_filepath: str) -> None:
        ...
        try:
            ...
            async with aiofiles.open(log_filepath, 'w') as f:
                for log in logs:
                    log_line = f"{log['timestamp']} [SECURITY] {log['event_type']}: {log['tech_info']}\n"
                    await f.write(log_line)
    ...
    async def _collect_firmware_status_logs(self, station_id: str, log_filepath: str) -> None:
        ...
        try:
            ...
            async with aiofiles.open(log_filepath, 'w') as f:
                for log in logs:
                    log_line = f"{log['timestamp']} [FIRMWARE] {log['status']}: {log['additional_info']}\n"
                    await f.write(log_line)
    ...
    async def _collect_local_list_logs(self, station_id: str, log_filepath: str) -> None:
        ...
        try:
            ...
            async with aiofiles.open(log_filepath, 'w') as f:
                for log in logs:
                    log_line = f"{log['timestamp']} [LOCAL_LIST] {log['action']}: {log['id_token']}\n"
                    await f.write(log_line)
    ...
    async def _handle_firmware_update_event(self, station_id: str, tech_info: Optional[str],
                                          additional_info: Optional[Dict[str, Any]]) -> None:
    ...
    async def _handle_security_event(self, station_id: str, tech_info: Optional[str],
                                   additional_info: Optional[Dict[str, Any]]) -> None:
    ...
    async def _handle_reset_event(self, station_id: str, event_type: str, tech_info: Optional[str]) -> None:
        """Handle reset event."""
        try:
            # Store reset event
...
class FirmwareManager:
    ...
    async def publish_firmware(self, station_id: str, location: str, retrieve_date_time: str,
                             request_id: int, retry_interval: Optional[int] = None,
                             retries: Optional[int] = None, 
                             retry_back_off_random_range: Optional[int] = None,
                             checksum: Optional[str] = None,
                             checksum_algorithm: Optional[str] = None,
                             ...
                             request_stop_time: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            firmware_info = FirmwareInfo(
                location=location,
                retrieve_date_time=datetime.fromisoformat(retrieve_date_time.replace('Z', '+00:00')),
            ...
            firmware_request = FirmwareUpdateRequest(
                ...
                request_start_time=datetime.fromisoformat(request_start_time.replace('Z', '+00:00')) if request_start_time else None,
                request_stop_time=datetime.fromisoformat(request_stop_time.replace('Z', '+00:00')) if request_stop_time else None
            )
            ...
            await self.timescale_client.store_firmware_request({
                ...
                "signing_certificate": signing_certificate,
                "signature": signature,
                "signing_certificate_chain": json.dumps(signing_certificate_chain) if signing_certificate_chain else None,
    ...
    async def handle_firmware_status_notification(self, station_id: str, status: str,
        ...
        try:
            ...
            # Remove from active requests if completed
            if status in [FirmwareStatus.INSTALLED.value, FirmwareStatus.INSTALLATION_FAILED.value,
    ...
    async def update_firmware(self, station_id: str, location: str, retrieve_date_time: str,
                            request_id: int, retry_interval: Optional[int] = None,
                            retries: Optional[int] = None,
                            retry_back_off_random_range: Optional[int] = None,
                            checksum: Optional[str] = None,
    ...
    async def _download_firmware(self, firmware_request: FirmwareUpdateRequest) -> None:
        ...
        try:
            ...
            # Download firmware
            firmware_data = await self._download_firmware_data(firmware_request.firmware_info.location)
            
            # Verify checksum if provided
            if firmware_request.firmware_info.checksum:
                if not await self._verify_firmware_checksum(firmware_data, firmware_request.firmware_info):
                    await self.timescale_client.update_firmware_request_status(
                        firmware_request.station_id, firmware_request.request_id,
                        FirmwareStatus.DOWNLOAD_FAILED.value, "Checksum verification failed"
                    )
                    return
            
            # Verify signature if provided
            if firmware_request.firmware_info.signature:
                if not await self._verify_firmware_signature(firmware_data, firmware_request.firmware_info):
                    await self.timescale_client.update_firmware_request_status(
                        firmware_request.station_id, firmware_request.request_id,
                        FirmwareStatus.INVALID_SIGNATURE.value, "Signature verification failed"
                    )
                    return
            ...
            await self.timescale_client.update_firmware_request_status(
                firmware_request.station_id, firmware_request.request_id,
                FirmwareStatus.DOWNLOAD_FAILED.value, str(e)
            )
    ...
    async def _download_and_install_firmware(self, firmware_request: FirmwareUpdateRequest) -> None:
        ...
        try:
            ...
            await self.timescale_client.update_firmware_request_status(
                firmware_request.station_id, firmware_request.request_id,
                FirmwareStatus.INSTALLATION_FAILED.value, str(e)
            )
    ...
    async def _verify_firmware_checksum(self, firmware_data: bytes, firmware_info: FirmwareInfo) -> bool:
        ...
        try:
            ...
            if firmware_info.checksum_algorithm.lower() == "sha256":
                ...
                self.logger.warning(f"Unsupported checksum algorithm: {firmware_info.checksum_algorithm}")
    ...
    async def _verify_firmware_signature(self, firmware_data: bytes, firmware_info: FirmwareInfo) -> bool:
    ...
    async def _handle_firmware_installed(self, station_id: str, request_id: Optional[int]) -> None:
        ...
        try:
            ...
            firmware_info = await self.timescale_client.get_firmware_request_info(station_id, request_id)
            if firmware_info:
    ...
    async def _handle_firmware_installation_failed(self, station_id: str, request_id: Optional[int]) -> None:
    ...
    async def _handle_firmware_download_failed(self, station_id: str, request_id: Optional[int]) -> None:
