...
class SupabaseClient:
    ...
    async def update_organization(self, org_id: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Update organization."""
        if not self.client:
            raise RuntimeError("Supabase client not initialized")
    ...
    async def update_vehicle_status(self, vehicle_id: str, status: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.client:
            raise RuntimeError("Supabase client not initialized")
        response = self.client.table('vehicles').update({'status': status}).eq('id', vehicle_id).execute()
        return response.data[0] if response.data else None
    ...
    async def update_station_status(self, station_id: str, status: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.client:
            raise RuntimeError("Supabase client not initialized")
        response = self.client.table('charging_stations').update({'status': status}).eq('id', station_id).execute()
        return response.data[0] if response.data else None
    ...
    async def update_session_status(self, session_id: str, status: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.client:
            raise RuntimeError("Supabase client not initialized")
        response = self.client.table('charging_sessions_active').update({'status': status}).eq('id', session_id).execute()
        return response.data[0] if response.data else None
    ...
    async def get_daily_energy_summary(self, org_id: str, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        """Get daily energy summary."""
        if not self.client:
            raise RuntimeError("Supabase client not initialized")
        response = self.client.table('daily_energy_summary').select('*').eq('organization_id', org_id).gte('date', start_date.isoformat()).lte('date', end_date.isoformat()).execute()
        return response.data if response.data else []
    
    async def calculate_savings(self, org_id: str, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        """Calculate savings."""
        if not self.client:
            raise RuntimeError("Supabase client not initialized")
