        ...
        if self.state == CircuitBreakerState.HALF_OPEN:
            if self.half_open_calls >= self.config.half_open_max_calls:
                ...
                raise CircuitBreakerOpenError(
                    f"Circuit breaker {self.name} exceeded half-open calls"
                )
    ...
    async def execute_with_retry(
        ...
        for attempt in range(retry_config.max_attempts):
            try:
                ...
                if not any(
                    isinstance(e, exc_type) for exc_type in retry_config.retryable_exceptions
                ):
                ...
                if attempt == retry_config.max_attempts - 1:
                    self.logger.error(
                        f"All {retry_config.max_attempts} retry attempts failed for {func.__name__}"
                    )
                    raise e
...
def with_bulkhead(name: str, config: Optional[BulkheadConfig] = None):
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            return await error_handler.execute_with_resilience(func, *args, bulkhead=name, **kwargs)
