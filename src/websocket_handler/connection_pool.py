...
from .config import TimescaleConfig, SupabaseConfig
...
class EnhancedConnectionPool:
    ...
    def __init__(self, config: Union[TimescaleConfig, SupabaseConfig], 
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Enhanced connection pool initialized with {self.min_connections}-{self.max_connections} connections")
    ...
    def get_pool_metrics(self) -> PoolMetrics:
        ...
        avg_connection_time = (self.metrics["total_connection_time"] / self.metrics["successful_connections"] 
                             if self.metrics["successful_connections"] > 0 else 0.0)
        
        hit_rate = (self.metrics["pool_hits"] / (self.metrics["pool_hits"] + self.metrics["pool_misses"]) 
                   if (self.metrics["pool_hits"] + self.metrics["pool_misses"]) > 0 else 0.0)
    ...
    async def optimize_pool(self) -> None:
        ...
        elif metrics.pool_hit_rate > 0.95 and metrics.active_connections > metrics.total_connections * 0.8:
    ...
    async def _monitor_pools(self) -> None:
        ...
        while self._running:
            try:
                ...
                # Log pool metrics
                metrics = self.get_pool_metrics()
                self.logger.info(f"Pool metrics: {metrics.total_connections} total, "
...
class ConnectionPoolManager:
    ...
    async def create_pool(self, name: str, config: Union[TimescaleConfig, SupabaseConfig],
                         strategy: PoolStrategy = PoolStrategy.HYBRID) -> EnhancedConnectionPool:
