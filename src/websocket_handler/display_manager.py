...
class DisplayManager:
    ...
    async def set_display_message(self, station_id: str, message_info: MessageInfoType,
        ...
        try:
            ...
            if not self._validate_message_info(message_info):
                return {
                    ...
                    "statusInfo": StatusInfoType(reason_code="InvalidMessage", 
                                               additional_info="Message validation failed")
                }
            ...
            # Store history
            await self._store_message_history(station_id, message_id, "created", {
    ...
    async def clear_display_message(self, station_id: str, message_id: Optional[str] = None,
                                  evse_id: Optional[int] = None,
                                  connector_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            if message_id:
                ...
                # Store history
                await self._store_message_history(station_id, message_id, "cleared", {
                    "cleared_by": "system"
                })
                ...
                # Store history
                await self._store_message_history(station_id, "all", "cleared", {
                    "evse_id": evse_id,
                    "connector_id": connector_id,
                    "cleared_by": "system"
                })
                ...
                for msg_id, msg_data in self.active_messages.items():
                    if (msg_data["station_id"] == station_id and
                        (evse_id is None or msg_data.get("evse_id") == evse_id) and
                        (connector_id is None or msg_data.get("connector_id") == connector_id)):
    ...
    async def get_display_messages(self, station_id: str, evse_id: Optional[int] = None,
                                 connector_id: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def _validate_message_info(self, message_info: MessageInfoType) -> bool:
        ...
        if message_info.display_message and message_info.display_message.start_date_time:
            if (message_info.display_message.end_date_time and 
                message_info.display_message.end_date_time <= message_info.display_message.start_date_time):
                return False
    ...
    async def _store_message_history(self, station_id: str, message_id: str, action: str,
    ...
    async def create_system_message(self, station_id: str, message_type: str, content: str,
                                  evse_id: Optional[int] = None,
                                  connector_id: Optional[int] = None,
                                  priority: int = 50,
                                  valid_duration: Optional[timedelta] = None) -> str:
    ...
    async def create_charging_message(self, station_id: str, evse_id: int, connector_id: int,
                                    message: str, message_type: str = "Info") -> str:
    ...
    async def create_error_message(self, station_id: str, error_message: str,
                                 evse_id: Optional[int] = None,
                                 connector_id: Optional[int] = None) -> str:
        """Create an error display message."""
        return await self.create_system_message(
            station_id, "Error", error_message, evse_id, connector_id, 100
        )

    async def create_warning_message(self, station_id: str, warning_message: str,
                                   evse_id: Optional[int] = None,
                                   connector_id: Optional[int] = None) -> str:
