...
class AdvancedMetering:
    ...
    async def process_meter_values(self, station_id: str, evse_id: int, 
                                 meter_value: List[Dict[str, Any]]) -> bool:
    ...
    async def get_energy_accounting(self, station_id: str, evse_id: int, 
                                  transaction_id: Optional[str] = None) -> Optional[EnergyAccounting]:
    ...
    async def get_power_quality_report(self, station_id: str, start_time: datetime, 
    ...
    async def _apply_meter_calibration(self, station_id: str, meter_value: MeterValue) -> MeterValue:
    ...
    async def _create_signed_meter_value(self, meter_value: MeterValue) -> SignedMeterValue:
        ...
        try:
            ...
            signature = hmac.new(
                self.meter_signing_key.encode(),
                signed_data_json.encode(),
                hashlib.sha256
            ).hexdigest()
    ...
    async def _store_signed_meter_value(self, station_id: str, evse_id: int, 
                                      signed_value: SignedMeterValue) -> None:
        """Store signed meter value."""
        try:
    ...
    async def _update_energy_accounting(self, station_id: str, evse_id: int, 
                                      meter_value: MeterValue) -> None:
        """Update energy accounting."""
        try:
    ...
    async def _check_power_quality(self, station_id: str, meter_value: MeterValue) -> None:
        ...
        try:
            ...
            if events:
                ...
                self.logger.warning(f"Power quality events detected for {station_id}: {[e.value for e in events]}")
            
        except Exception as e:
            self.logger.error(f"Error checking power quality: {e}")
    
    async def _extract_power_quality_reading(self, meter_value: MeterValue) -> Optional[PowerQualityReading]:
    ...
    async def _detect_power_quality_events(self, pq_reading: PowerQualityReading) -> List[PowerQualityEvent]:
        ...
        try:
            ...
            if pq_reading.frequency:
                if (pq_reading.frequency < self.power_quality_thresholds["frequency_min"] or
                    pq_reading.frequency > self.power_quality_thresholds["frequency_max"]):
                    events.append(PowerQualityEvent.FREQUENCY_DEVIATION)
            
            # Check harmonic distortion
            if pq_reading.thd_voltage and pq_reading.thd_voltage > self.power_quality_thresholds["thd_voltage_max"]:
                events.append(PowerQualityEvent.HARMONIC_DISTORTION)
            
            # Check power factor
            if pq_reading.power_factor and pq_reading.power_factor < self.power_quality_thresholds["power_factor_min"]:
                events.append(PowerQualityEvent.POWER_FACTOR_LOW)
            
            # Check phase imbalance
            if pq_reading.phase_imbalance and pq_reading.phase_imbalance > self.power_quality_thresholds["phase_imbalance_max"]:
    ...
    async def _calculate_power_quality_statistics(self, readings: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            voltages_l2 = [r["voltage_l2"] for r in readings if r.get("voltage_l2")]
            voltages_l3 = [r["voltage_l3"] for r in readings if r.get("voltage_l3")]
    ...
    async def _identify_power_quality_events(self, readings: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _check_power_quality_compliance(self, statistics: Dict[str, Any]) -> Dict[str, bool]:
        ...
        try:
            ...
            if "voltage_l1" in statistics:
                ...
                compliance["voltage"] = (
                    voltage_stats["min"] >= self.power_quality_thresholds["voltage_min"] and
                    voltage_stats["max"] <= self.power_quality_thresholds["voltage_max"]
                )
            ...
            if "frequency" in statistics:
                ...
                compliance["frequency"] = (
                    frequency_stats["min"] >= self.power_quality_thresholds["frequency_min"] and
                    frequency_stats["max"] <= self.power_quality_thresholds["frequency_max"]
                )
            
            # Check THD compliance
            if "thd_voltage" in statistics:
                thd_stats = statistics["thd_voltage"]
                compliance["thd_voltage"] = thd_stats["max"] <= self.power_quality_thresholds["thd_voltage_max"]
            
            # Check power factor compliance
            if "power_factor" in statistics:
                pf_stats = statistics["power_factor"]
                compliance["power_factor"] = pf_stats["min"] >= self.power_quality_thresholds["power_factor_min"]
            
            # Check phase imbalance compliance
            if "phase_imbalance" in statistics:
                imbalance_stats = statistics["phase_imbalance"]
                compliance["phase_imbalance"] = imbalance_stats["max"] <= self.power_quality_thresholds["phase_imbalance_max"]
