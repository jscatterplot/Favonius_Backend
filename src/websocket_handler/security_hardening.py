...
class CertificateManager:
    ...
    def validate_certificate(self, cert: x509.Certificate) -> Dict[str, Any]:
        ...
        if isinstance(public_key, rsa.RSAPublicKey):
            ...
            if key_size < 2048:
                validation_result["warnings"].append(f"RSA key size {key_size} is less than 2048 bits")
...
class TLSServer:
    ...
    def _generate_development_certificate(self) -> None:
        ...
        cert, private_key = self.cert_manager.generate_self_signed_certificate(
            ...
            organization="EV Charging V2G Development",
            validity_days=365
        )
    ...
    def validate_tls_configuration(self) -> Dict[str, Any]:
        ...
        if self.tls_config.cert_path:
            if not os.path.exists(self.tls_config.cert_path):
                validation_result["valid"] = False
                validation_result["errors"].append(f"Certificate file not found: {self.tls_config.cert_path}")
            else:
        ...
        if self.tls_config.key_path:
            if not os.path.exists(self.tls_config.key_path):
                validation_result["valid"] = False
                validation_result["errors"].append(f"Private key file not found: {self.tls_config.key_path}")
            else:
                # Check file permissions
                key_stat = os.stat(self.tls_config.key_path)
                if key_stat.st_mode & 0o077:  # Check if readable by others
                    validation_result["warnings"].append("Private key file has overly permissive permissions")
        
        # Check CA certificate file
        if self.tls_config.ca_path:
            if not os.path.exists(self.tls_config.ca_path):
                validation_result["valid"] = False
                validation_result["errors"].append(f"CA certificate file not found: {self.tls_config.ca_path}")
...
class SecurityAuditor:
    ...
    def _generate_security_recommendations(self, audit_results: Dict[str, Any]) -> List[str]:
        ...
        if overall_score < 80:
            recommendations.append("Overall security score is below 80%. Address critical issues immediately.")
...
class SecurityManager:
    ...
    async def initialize_security(self) -> Dict[str, Any]:
        ...
        try:
            # Initialize TLS
            ssl_context = self.tls_server.create_ssl_context()
