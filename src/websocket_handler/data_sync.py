...
class DataSyncService:
    ...
    async def sync_charging_sessions(self) -> None:
        ...
        try:
            ...
            # Get last sync time
            last_sync = self.last_sync_times.get('charging_sessions', datetime.now(timezone.utc) - timedelta(hours=1))
                ...
                if sessions:
                    ...
                    for session in sessions:
                        session_data.append({
                            ...
                            'energy_delivered_kwh': float(session['energy_delivered_kwh']) if session['energy_delivered_kwh'] else 0,
                            'energy_received_kwh': float(session['energy_received_kwh']) if session['energy_received_kwh'] else 0,
                            'session_duration_minutes': session['session_duration_minutes'],
                            'cost_total': float(session['cost_total']) if session['cost_total'] else 0,
                            'revenue_v2g': float(session['revenue_v2g']) if session['revenue_v2g'] else 0,
    ...
    async def sync_vehicle_states(self) -> None:
        ...
        try:
            ...
            # Get last sync time
            last_sync = self.last_sync_times.get('vehicle_states', datetime.now(timezone.utc) - timedelta(minutes=5))
                ...
                if states:
                    ...
                    for state in states:
                        state_data.append({
                            ...
                            'current_soc': float(state['current_soc']) if state['current_soc'] else 0,
                            'current_power_kw': float(state['current_power_kw']) if state['current_power_kw'] else 0,
                            'charging_status': state['charging_status'],
                            'location': f"POINT({state['location_lon']} {state['location_lat']})" if state['location_lat'] and state['location_lon'] else None,
    ...
    async def sync_optimization_decisions(self) -> None:
        ...
        try:
            ...
            # Get last sync time
            last_sync = self.last_sync_times.get('optimization_decisions', datetime.now(timezone.utc) - timedelta(minutes=1))
                ...
                if decisions:
                    ...
                    for decision in decisions:
                        decision_data.append({
                            ...
                            'power_setpoint_kw': float(decision['power_setpoint_kw']) if decision['power_setpoint_kw'] else 0,
                            'duration_minutes': decision['duration_minutes'],
                            'cost_savings': float(decision['cost_savings']) if decision['cost_savings'] else 0,
    ...
    async def sync_energy_metrics(self) -> None:
        ...
        try:
            ...
            # Get last sync time
            last_sync = self.last_sync_times.get('energy_metrics', datetime.now(timezone.utc) - timedelta(hours=1))
                ...
                if metrics:
                    ...
                    for metric in metrics:
                        await self.supabase_client.client.table('daily_energy_summary').upsert({
                            ...
                            'total_energy_charged': float(metric['total_energy_charged']) if metric['total_energy_charged'] else 0,
                            'total_energy_discharged': float(metric['total_energy_discharged']) if metric['total_energy_discharged'] else 0,
                            'avg_session_duration': float(metric['avg_session_duration']) if metric['avg_session_duration'] else 0,
                            'total_cost': float(metric['total_cost']) if metric['total_cost'] else 0,
                            'total_v2g_revenue': float(metric['total_v2g_revenue']) if metric['total_v2g_revenue'] else 0
                        }).execute()
        ...
        try:
            ...
            for session in sessions:
                session_data.append({
                    ...
                    'estimated_end_time': session.get('estimated_end_time').isoformat() if session.get('estimated_end_time') else None,
                    'status': session.get('status', 'charging')
                })
            
            # Upsert to Supabase
            await self.supabase_client.client.table('charging_sessions_active').upsert(session_data).execute()
