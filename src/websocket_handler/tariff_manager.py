...
class TariffManager:
        ...
        try:
            ...
            if not self._validate_tariff_data(tariff_data):
                return {
                    ...
                    "statusInfo": StatusInfoType(reason_code="InvalidTariff", 
                                               additional_info="Tariff validation failed")
                }
    ...
    async def calculate_transaction_cost(self, station_id: str, transaction_id: str,
                                       energy_kwh: float, duration_seconds: int,
                                       max_power_kw: float = 0.0) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Calculated cost for transaction {transaction_id}: {total_cost} {primary_tariff['tariff_currency']}")
    ...
    async def create_cost_updated_notification(self, station_id: str, transaction_id: str,
        ...
        try:
            ...
            for cost_type, amount in cost_data.get("cost_breakdown", {}).items():
                cost_kind = CostKindEnumType.carbon_dioxide_emission  # Default
                
                if cost_type == "energy_cost":
                    cost_kind = CostKindEnumType.energy_cost
                elif cost_type == "time_cost":
                    cost_kind = CostKindEnumType.time_cost
                elif cost_type == "power_cost":
                    cost_kind = CostKindEnumType.power_cost
            ...
            consumed_cost = ConsumedCostType(
                cost=costs,
                total_cost=cost_data["total_cost"]
            )
            
            # Create cost updated notification
            cost_updated = CostUpdatedType(
                evse_id=evse_id,
    ...
    async def create_sales_tariff(self, station_id: str, tariff_data: Dict[str, Any]) -> SalesTariffType:
        ...
        try:
            ...
            for period in periods:
                ...
                period_elements = [e for e in elements if e["element_type"] in ["Energy", "Time", "Power"]]
                ...
                if period_elements:
                    ...
                    time_interval = RelativeTimeIntervalType(
                        start=start_time,
                        duration=duration
                    )
                    ...
                    entry = SalesTariffEntryType(
                        relative_time_interval=time_interval,
                        e_price_level=1  # Default price level
                    )
    ...
    async def get_cost_history(self, station_id: str, transaction_id: Optional[str] = None,
    ...
    async def _calculate_default_cost(self, energy_kwh: float, duration_seconds: int) -> Dict[str, Any]:
    ...
    async def create_time_of_use_tariff(self, station_id: str, peak_price: float = 0.30,
                                      off_peak_price: float = 0.15) -> str:
