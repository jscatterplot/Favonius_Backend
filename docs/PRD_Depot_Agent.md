...
- v0.2 — Sprint 4 readiness substrate. Adds the data plumbing the daily readiness workflow needs: `schedules.driver_id` (Supabase migration 013) hangs a per-route driver assignment off the existing schedules row, and the `routes` view (Supabase migration 014) gives the agent a stable Route shape aliased over `schedules` until a customer-side routes system ships. Ships the five typed tools §6.1 lists (`get_scheduled_departures`, `get_vehicle_state`, `get_charger_state`, `get_charging_plan`, `get_driver_assignment`) under `src/api/agent_workflows/tools/readiness.py`, all auth-scoped to `AuthContext.visible_depot_ids` and using `src.security.data_freshness.MAX_TELEMETRY_AGE` (15 min) as the canonical telemetry staleness threshold — the same constant the optimizer's `StateAssembler` enforces. Tools never raise on empty data; they return empty lists or `None`-bearing dicts with explicit freshness flags so the orchestrator can reason about gaps.
