...
- v0.1.1 (sprint 3) — Workflow evaluation harness landed ahead of any workflow code, satisfying principle 6 ("the evaluation harness ships before the workflow"). The harness lives in `src/api/agent_workflows/eval/`; scenarios live in `tests/golden/workflows/` against the schema in `_schema.yaml`. Three placeholder scenarios under `_examples/` prove the harness end-to-end. The CI gate (`.github/workflows/workflow-golden.yml`) enforces a runner-coverage floor of 90% (currently 100%) and blocks PRs whose YAML scenarios marked `severity: blocking` fail. Sprint 5 will land the first 10 readiness scenarios under the gated path; sprint 4 wires the workflow runtime past the stub in `src/api/agent_workflows/runtime.py`.
