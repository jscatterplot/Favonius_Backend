...
- v0.2 — Sprint 6 (Today View) shipped. Added `/agent-workflows/*` HTTP surface (4 endpoints), `daily_readiness_check` workflow at `src/core/workflows/readiness.py` producing the §6.1 payload, `WorkflowScheduler` triggering the workflow lead-time-minutes before the earliest scheduled departure per depot, and the `workflow_decisions` audit table (migration 037). All gated by `DEPOT_AGENT_ENABLED` (default off). Prometheus metrics: `favonius_agent_workflow_requests_total`, `favonius_agent_workflow_runs_total`, `favonius_agent_workflow_run_duration_seconds`, `favonius_agent_workflow_scheduler_to_completion_seconds`, `favonius_agent_workflow_exceptions_emitted_total`. Test suite: unit (readiness logic, scheduler, rate-limit bucket, feature flag, router auth), integration (scenario 02 — one undercharge), e2e (full happy-path SSE stream). Production stays off until the scenarios + a one-week shadow run against pilot data pass; flipped to `true` in staging only.
