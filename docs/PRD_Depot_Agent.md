...
- Sprint 2 (runtime) — Adds the workflow runtime on top of the Sprint 1 substrate:
  - `WorkflowAgent.run_turn` (`src/api/agent_workflows/runtime.py`) drives one Anthropic Messages-API tool-use loop per turn, enforces `workflow.allowed_tools` strictly before each dispatch, and persists exactly one `Decision` row via the Sprint 1 `insert_decision` writer (wrapped behind `DecisionRepo`). Disposition is always `Disposition.PENDING` — the agent itself never writes `auto_executed` (PRD §9.2, §10.4).
  - `ToolRegistry` (`src/api/agent_workflows/tools.py`) is the single source of truth for tool name → JSON schema + async callable. The runtime never looks up a callable elsewhere.
  - `HardConstraintGuard` (`src/api/agent_workflows/constraints.py`) enforces the substrate hard constraints (PRD §10.3) at two points: pre-dispatch on tool inputs, and post-emit on the LLM's `proposed_actions`. Violations are recorded in the audit `output`, never executed.
  - The system prompt is prompt-cached (`cache_control: ephemeral`) so repeat turns of the same workflow hit Anthropic's prefix cache, mirroring the pattern in `src/api/agent/llm.py`. Per-turn payload (depot id, parameters, user input) lives in the user message so the prefix cache stays hot.
  - Per-(workflow, depot) `permission_tier` is read from `workflow_tiers` via Sprint 1's `get_tier(...)` by the caller and threaded into `run_turn(..., permission_tier=...)`. The runtime defaults to `DRAFT_AND_WAIT` if no tier is configured (PRD §9.2).
...
- Sprint 3 (eval harness) — Lands the evaluation harness ahead of any workflow definition, satisfying principle 6 ("the evaluation harness ships before the workflow"). The harness in `src/api/agent_workflows/eval/runner.py` drives Sprint 2's `WorkflowAgent.run_turn` against a frozen `graph_snapshot` loaded into a transactional savepoint on the real test DB (no mocked DDL — schema drift surfaces in CI). Each scenario YAML carries a `workflow` block (name, version, prompt, allowed_tools), a `graph_snapshot`, a deterministic `llm_trace` the harness replays through a fake `AnthropicClient`, and an `expected` block asserting against `Decision.output` + `Decision.tool_calls`. Non-deterministic fields (`Decision.id`, `timestamp`, `inputs_hash`) are excluded from assertions. Scenarios live as YAML under `tests/golden/workflows/` against `_schema.yaml`; three placeholder scenarios under `_examples/` prove the harness end-to-end. CI gate: `.github/workflows/workflow-golden.yml` spins up TimescaleDB, applies migrations, runs the harness, and enforces a 90% runner coverage floor. Sprint 5 will land the first 10 readiness scenarios under the gated path.
- Sprint 4 (readiness tools) — Substrate the daily-readiness workflow runs on. Two Supabase migrations land the missing data plumbing: `013_schedules_driver_id.sql` adds a nullable `schedules.driver_id` FK + partial index, and `014_routes_view.sql` exposes the PRD §5.1 `Route` shape (`id`, `energy_estimate_kwh`, `start_time`, `end_time`, `contract_id`) as a view aliased over `schedules` until a customer-side routes system ships. `src/api/agent_workflows/readiness_tools.py::build_readiness_tool_registry(static_pool, ts_pool, auth, *, depot_id, now=None)` builds a Sprint-2-shaped `ToolRegistry` populated with the five tools §6.1 lists (`get_scheduled_departures`, `get_vehicle_state`, `get_charger_state`, `get_charging_plan`, `get_driver_assignment`). Each tool takes `**kwargs` matching its JSON input schema; auth, pools, and the caller's visible-depot scope are captured at registry-build time via closures, so `await registry.dispatch(name, llm_input)` from Sprint 2's runtime works unchanged. Auth scoping is by `auth.visible_depot_ids` against `sites.organization_id` (same boundary as `src/api/agent/resolve.py`); telemetry staleness uses `src.security.data_freshness.MAX_TELEMETRY_AGE` (15 min), the same constant the optimizer's `StateAssembler` enforces. Tools never raise on empty data; they return `[]` or `None`-bearing dicts with explicit freshness flags so the runtime can reason about gaps without aborting the turn.
- Sprint 5 (readiness workflow registration) — The first workflow to do real work. `src/api/agent_workflows/readiness_workflow.py` defines the `daily_readiness_check` workflow as code: a single source-of-truth system prompt encoding the §6.1 logic and the §10.3 hard-constraint guardrails the LLM must respect (no proposal below 99% of departure SoC, none that would violate `max_grid_kw`, escape hatch is `requires_manager: true`), the five sprint-4 tool names in `allowed_tools`, and the three tier-1-openness parameters (`lead_time_min`, `soc_tolerance_pct`, `escalation_threshold`). A new `repository.upsert_workflow(...)` writer plus `register_daily_readiness_workflow(...)` startup hook upsert the row into `workflows` and seed one `workflow_tiers` row at `inform` for every visible depot on every cold start — idempotent, never silently graduating existing rows. Wired into `src/api/main.py` lifespan behind `DEPOT_AGENT_ENABLED`. 10 frozen scenarios under `tests/golden/workflows/` (`01_all_clear`, `02_one_undercharge`, `03_faulted_charger`, `04_no_driver`, `05_driver_shift_conflict`, `06_stale_telemetry`, `07_constraint_violation_test`, `08_late_return`, `09_dst_transition`, `10_cascading_charger_faults`) drive the sprint-3 harness end-to-end against the runtime + `HardConstraintGuard`. AT-D1 in `tests/e2e/test_depot_agent_readiness.py` exercises the full upsert → run → persisted `Decision` flow. Tier remains `inform` at end of sprint per PRD §9.2; graduation to `draft_and_wait` blocks on the metrics-out criteria in PRD §11.1.
