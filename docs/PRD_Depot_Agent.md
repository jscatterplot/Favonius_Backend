...
- v0.2 — Sprint 2 (depot-agent/sprint-2-runtime). Implemented the workflow agent runtime in `src/api/agent_workflows/`: `WorkflowAgent.run_turn` wraps the Anthropic Messages API with tool-use, validates each call against `workflow.allowed_tools` before dispatch, filters the LLM's structured output through `HardConstraintGuard` (PRD §10.3: departure SoC ≥ 99%, `max_grid_kw` cap), and writes one immutable `Decision` row per turn via the repo (migration `037_decisions.sql` adds `UPDATE`/`DELETE` triggers for §10.4 audit immutability). System-prompt block is `cache_control: ephemeral`, mirroring `src/api/agent/llm.py`. Reserved `submit_decision` tool name is the structured-output channel — auto-added to every workflow's Anthropic tool array, never part of `allowed_tools`. Prometheus metrics: `favonius_workflow_turns_total{workflow,depot,status}`, `favonius_workflow_turn_duration_seconds{workflow}`, `favonius_workflow_llm_tokens_total{workflow,model,direction}`. No HTTP endpoint shipped yet; the runtime is exercised by `tests/unit/test_agent_workflows_runtime.py` (100% coverage). Note: the `Workflow` and `Decision` pydantic schemas referenced as sprint-1 deliverables landed alongside this sprint in the same PR — sprint-1 did not produce a separate branch.
