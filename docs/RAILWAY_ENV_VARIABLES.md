...
| **MAXMIND_ACCOUNT_ID** | — | **Secret.** Required since MaxMind's 2024 policy change — paired with `MAXMIND_LICENSE_KEY` as the username in HTTP Basic Auth against the GeoLite2 download endpoint. Set as a **Service** (runtime) variable only — never as a Build variable, since Docker build args are recorded in image history and visible in build logs. Get from maxmind.com → Account → Account Information. |
| **MAXMIND_LICENSE_KEY** | — | **Secret.** MaxMind license key for the GeoLite2-Country DB used by the Article 73-3 geo-blocking middleware. Set as a **Service** (runtime) variable only — `src/security/geo_block.py::_download_geoip_db` downloads the DB on startup with retries (linear backoff). Requires `MAXMIND_ACCOUNT_ID`; without either set, the service starts with no GeoIP DB and fails closed (every non-private IP gets a 403 “Access denied”). Get a key for free from maxmind.com → Account → License Keys. |
